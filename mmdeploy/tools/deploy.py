...
def main():
    ...
    deploy_cfg_path = args.deploy_cfg # 'mmdeploy\configs\mmdet\detection\detection_tensorrt_dynamic-320x320-1344x1344.py'
    model_cfg_path = args.model_cfg # '..//mmdetection/configs/faster_rcnn_r50_fpn_1x_coco.py'
    checkpoint_path = args.checkpoint # 'checkpoint/faster_rcnn_res50.pth'
    quant = args.quant # False
    quant_image_dir = args.quant_image_dir # None

    # load deploy_cfg 加载depoly和model配置文件
    ...
    if args.dump_info:
        export2SDK(
            ...
            args.work_dir,
            pth=checkpoint_path,
            device=args.device) # 导出detail.json, pipeline.json和detail.json文件

    ret_value = mp.Value('d', 0, lock=False) # 0

    # convert to IR
    ir_config = get_ir_config(deploy_cfg)
    ir_save_file = ir_config['save_file'] # 'end2end.onnx'
    ir_type = IR.get(ir_config['type']) # 'onnx'
    torch2ir(ir_type)(
        args.img,
        args.work_dir,
        ir_save_file,
        ...
        model_cfg_path,
        checkpoint_path,
        device=args.device) # torch2onnx(args, **kwargs)-->将模型转换为onnx模型

    # convert backend
    ir_files = [osp.join(args.work_dir, ir_save_file)] # 'mmdeploy_model/faster-rcnn/ene2end.onnx'
