...
def main():
    ...
    dataset_type = 'test'
    dataset = task_processor.build_dataset(model_cfg, dataset_type) # 构建dataset
    ...
    data_loader = task_processor.build_dataloader(
        ...
        workers_per_gpu=model_cfg.data.workers_per_gpu) # 构建data_loader

    # load the model of the backend
    model = task_processor.init_backend_model(args.model, uri=args.uri)

    is_device_cpu = (args.device == 'cpu') # False
    device_id = None if is_device_cpu else parse_device_id(args.device) # 0

    destroy_model = model.destroy # TRTWraper
    ...
    if args.speed_test:
        ...
        outputs = task_processor.single_gpu_test(model, data_loader, args.show,
                                                 args.show_dir) # 调用codebase的评估代码进行infer
    ...
    task_processor.evaluate_outputs(
        ...
        args.format_only,
        args.log2file,
        json_file=args.json_file) # 调用codebase的评估代码进行eval
