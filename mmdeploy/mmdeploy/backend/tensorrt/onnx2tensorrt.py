...
def onnx2tensorrt(work_dir: str,
    ...
    deploy_cfg = load_config(deploy_cfg)[0] # 部署配置文件

    mmcv.mkdir_or_exist(osp.abspath(work_dir)) # 创建输出文件夹

    common_params = get_common_config(deploy_cfg) # {'fp16_mode', 'max_workspace_size'}
    ...
    device_id = parse_device_id(device) # 0
    assert save_file.endswith(
        '.engine'
    ), 'Expect save file ends with `.engine`.' f' but get {save_file}'
    save_path = osp.join(work_dir, save_file) # 拼接输出路径 eg:'mmdeploy_model/faster-rcnn/end2end.engine'
    from_onnx(
        onnx_model,
