...
class TensorRTManager(BaseBackendManager):
    ...
    def to_backend(cls,
        ...
        model_params = get_model_inputs(deploy_cfg) # 获取模型输入参数
        ...
        for model_id, model_param, onnx_path in zip(
            ...
            onnx_name = osp.splitext(osp.split(onnx_path)[1])[0] # end2end
            save_file = model_param.get('save_file', onnx_name + '.engine') # end2end.engine

            partition_type = 'end2end' if partition_cfgs is None \
                else onnx_name # end2end
            onnx2tensorrt(
                work_dir,
                save_file,
                model_id,
