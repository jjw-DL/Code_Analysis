...
class HDF5Calibrator(trt.IInt8Calibrator):
    ...
    def __init__(
        ...
        calib_data = calib_file['calib_data'] # /calib_data
        assert model_type in calib_data
        calib_data = calib_data[model_type]

        self.calib_file = calib_file # calib_data.h5
        self.calib_data = calib_data # /calib_data/end2end
        self.device_id = device_id # 0
        self.algorithm = algorithm # trt.CalibrationAlgoType.ENTROPY_CALIBRATION_2
        self.input_shapes = input_shapes # 7个输入
        self.kwargs = kwargs
        ...
        self.count = 0
        first_input_group = calib_data[list(calib_data.keys())[0]] # /calib_data/end2end/img
        self.dataset_length = len(first_input_group) # 6109
        self.batch_size = first_input_group['0'].shape[0] # 6
