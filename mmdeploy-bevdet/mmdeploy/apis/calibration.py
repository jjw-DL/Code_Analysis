...
def create_calib_input_data(calib_file: str,
    ...
    with no_mp():
        ...
        # mmdeploy.codebase.mmdet3d.deploy.voxel_detection.VoxelDetection类
        task_processor = build_task_processor(model_cfg, deploy_cfg, device)

        apply_marks = cfg_apply_marks(deploy_cfg)

        model = task_processor.init_pytorch_model(model_checkpoint) # 初始化模型
        dataset = task_processor.build_dataset(dataset_cfg, dataset_type) # 构建数据集(根据不同的codebase进行构建)

        # patch model
        backend = get_backend(deploy_cfg).value # trt
        ir = IR.get(get_ir_config(deploy_cfg)['type']) # onnx
        patched_model = patch_model(
            model, cfg=deploy_cfg, backend=backend, ir=ir)

        dataloader = task_processor.build_dataloader(
            dataset, 1, 1, dist=False, shuffle=False) # 先进入task基类，然后注册器获取子类进而调用子类对应函数
        patched_model = MMDataParallel(patched_model, device_ids=[device_id])

        # 参数准备完毕后，正式进入calib的实现
