...
class AttentionConstrainedLoss(nn.Module):
    def __init__(self,
        ...
        self.ww_l = torch.LongTensor(ww).to(torch.cuda.current_device()) # (1600,)
        self.hh_l = torch.LongTensor(hh).to(torch.cuda.current_device()) # (1600,)
        ww = torch.FloatTensor(ww).to(torch.cuda.current_device()) + 0.5 # (1600,)
        hh = torch.FloatTensor(hh).to(torch.cuda.current_device()) + 0.5 # (1600,)
    ...
    def find_grid_in_bbx_single(self,
        ...
        gt_bboxes = torch.from_numpy(gt_bboxes).cuda() # (17, 9)
        if len(gt_bboxes > 0):
            temp_grid_flag = -1 * torch.ones(query_res, dtype=torch.long).cuda() # (40, 40) 全部初始化-1
            ...
            for i in range(len(gt_bboxes)):
                pos_mask = points_in_convex_polygon_torch(
                    grid_real_centers, effective_boxes[i].unsqueeze(0))  # (1600, 1)
                pos_ind = pos_mask.nonzero()[:, 0] # eg: [821, 861]-->(2,)
                gt_center = gt_bboxes[i: i + 1, :2]  # (1, 2)
                dist_to_grid_center = torch.norm(grid_real_centers - gt_center, dim=1)  # [W * H] (1600,)
    ...
    def forward(self,
                ...
                **kwargs):
        ret_dict = {}
        # List[4] 4:batch size, 每个元素包含n个tensor，表示n个box包含的atten点索引
        # loss的计算也是逐任务处理，任务内按照batch处理
        batch_grid_ind_list = list(self.find_grid_in_bbx(gt_bboxes)) 
        var_loss = torch.tensor(0.0).cuda() # 初始化0
        var_pos_num = 0.0
        # 逐帧处理
        for i in range(len(gt_bboxes)):
        ...
        ret_dict["var_loss"] = norm_var_loss # 记录loss
        return ret_dict
