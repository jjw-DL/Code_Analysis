...
class WeightedL1Loss(nn.Module):
    ...
    def __init__(
        ...
        self._loss_weight = loss_weight # 0.25
    ...
    def forward(self, prediction_tensor, target_tensor, weights=None):
        ...
        """
        diff = prediction_tensor - target_tensor # 对应位置相减 --> (651, 10)
        if self._code_weights is not None:
            diff = self._code_weights.view(1, 1, -1).to(diff.device) * diff # (1, 1, 10) * (651, 10)-->(1, 651, 10)
        loss = torch.abs(diff) # (1, 651, 10)
...
class SigmoidFocalLoss(nn.Module):
    ...
    def __init__(self, gamma=2.0, alpha=0.25, reduction="mean", loss_weight=1.0):
        ...
        self._alpha = alpha # 0.25
        self._gamma = gamma # 2.0
        self._reduction = reduction # 'mean'
    ...
    def forward(
        ...
        prediction_probabilities = torch.sigmoid(prediction_tensor) # (4, 25600, 1)
        p_t = (target_tensor * prediction_probabilities) + (
            (1 - target_tensor) * (1 - prediction_probabilities)
        ) # (4, 25600, 1)
        modulating_factor = 1.0
        if self._gamma:
            modulating_factor = torch.pow(1.0 - p_t, self._gamma) # (4, 25600, 1)
        ...
        weights = weights.unsqueeze(2) # (4, 25600, 1)
