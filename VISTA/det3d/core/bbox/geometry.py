...
def points_in_convex_polygon_torch(points, polygon, clockwise=True):
    ...
    try:
        polygon_next = torch.cat([polygon[:, -1:, :], polygon[:, :-1, :]], dim=1) # (1, 4, 2) 错开一个位置
    except:
        import pdb; pdb.set_trace()
    if clockwise:
        vec1 = (polygon - polygon_next) # 顺时针减少 (1, 4, 2)
    else:
        vec1 = (polygon_next - polygon)
    vec1 = vec1.unsqueeze(0) # (1, 1, 4, 2)
    vec2 = polygon.unsqueeze(0) - points.unsqueeze(1).unsqueeze(1) # (1, 1, 4, 2) - (1600, 1, 1, 2) --> (1600, 1, 4, 2)
    #vec2 = vec2[..., [1, 0]]
    vec2 = vec2.flip(-1)
    vec2[..., 1] *= -1
    cross = (vec1 * vec2).sum(-1) # (1600, 1, 4)
