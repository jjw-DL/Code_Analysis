...
class NuScenesDataset(Custom3DDataset):
    ...
    def __init__(self,
        ...
        self.img_info_prototype = img_info_prototype # info 原型

        self.speed_mode = speed_mode # 'relative_dis'
        self.max_interval = max_interval # 3
        self.min_interval = min_interval # 0
        self.prev_only = prev_only # 默认False True
        self.next_only = next_only # 默认False False
        self.test_adj = test_adj # 'prev'
        self.fix_direction = fix_direction # 默认False
        self.test_adj_ids = test_adj_ids # 默认None
    ...
    def get_data_info(self, index):
        ...
        if self.modality['use_camera']:
            if self.img_info_prototype == 'mmcv':
                ...
                input_dict.update(dict(img_info=info['cams'])) # 记录cams info的原始信息, 直接读取即可
            elif self.img_info_prototype == 'bevdet_sequential':
                if info ['prev'] is None or info['next'] is None:
                    adjacent= 'prev' if info['next'] is None else 'next' # 相邻帧名称
                else:
                    if self.prev_only or self.next_only: # self.prev_only = True
                        adjacent = 'prev' if self.prev_only else 'next' # 只使用前面的相邻帧
                    elif self.test_mode: # False
                        adjacent = self.test_adj # 'prev'
                    else:
                    ...
                    if self.test_mode:
                        ...
                        if len(info[adjacent])<= self.min_interval:
                            ...
                            select_id = np.random.choice([adj_id for adj_id in range(
                                min(self.min_interval,len(info[adjacent])), # 2
                                min(self.max_interval,len(info[adjacent])))]) # 9
                        info_adj = info[adjacent][select_id] # 获取相邻帧的info
                else:
                    info_adj = info[adjacent]
                input_dict.update(dict(img_info=info['cams'],
                                       curr=info, # 当前帧的info，增加curr字段
                                       adjacent=info_adj, # 增加adjacent字段，相邻帧的info:timestamp, cams, ego2global_rotation, ego2global_translation
                                       adjacent_type=adjacent)) # 相邻帧的类型

        if not self.test_mode:
            annos = self.get_ann_info(index)
            input_dict['ann_info'] = annos # 在input dict中增加标注信息：gt_bboxes_3d, gt_labels_3d, gt_names
            if self.img_info_prototype == 'bevdet_sequential':
                bbox = input_dict['ann_info']['gt_bboxes_3d'].tensor # 获取gt box
                if 'abs' in self.speed_mode:
                    bbox[:, 7:9] = bbox[:, 7:9] + torch.from_numpy(info['velo']).view(1,2) # 绝对速度，box速度+自车速度(lidar系下) eg:(42, 2)
                if input_dict['adjacent_type'] == 'next' and not self.fix_direction:
                    bbox[:, 7:9] = -bbox[:, 7:9]
                if 'dis' in self.speed_mode: # relative_dis 相对距离
                    time = abs(input_dict['timestamp'] - 1e-6 * input_dict['adjacent']['timestamp']) # 计算当前帧和相邻帧的时间差
                    bbox[:, 7:9] = bbox[:, 7:9] * time # 两个相邻帧之间的偏移量 --> 将速度预测变为位移
                input_dict['ann_info']['gt_bboxes_3d'] = LiDARInstance3DBoxes(bbox,
                                                                              box_dim=bbox.shape[-1],
                                                                              origin=(0.5, 0.5, 0.0)) # 构造lidarInstance3DBoxes
        return input_dict
