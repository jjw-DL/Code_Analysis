...
class RandomFlip3D(RandomFlip):
    ...
    def update_transform(self, input_dict):
        ...
        if input_dict['pcd_horizontal_flip']:
            aug_transform[1,1] = -1 # 水平翻转就是y变相反数
        if input_dict['pcd_vertical_flip']:
            aug_transform[0,0] = -1 # 垂直翻转就是x变相反数
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        input_dict['points'].translate(trans_factor)
        input_dict['pcd_trans'] = trans_factor # 增加平移因子
        for key in input_dict['bbox3d_fields']:
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        for key in input_dict['bbox3d_fields']:
            if len(input_dict[key].tensor) != 0:
                ...
                input_dict['points'] = points
                input_dict['pcd_rotation'] = rot_mat_T # 增加旋转矩阵信息
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        Returns:
            dict: Results after scaling, 'points'and keys in \
                input_dict['bbox3d_fields'] are updated in the result dict.
        """
        scale = input_dict['pcd_scale_factor'] # 提取缩放因子
        points = input_dict['points'] # 提取点云
    ...
    def update_transform(self, input_dict):
        # 初始化变换矩阵
        transform = torch.zeros((input_dict['img_inputs'][1].shape[0],4,4)).float() # (6, 4, 4)
        transform[:,:3,:3] = input_dict['img_inputs'][1] # (6, 3, 3)
        transform[:,:3,-1] = input_dict['img_inputs'][2] # (6, 3)
        transform[:, -1, -1] = 1.0
        # 根据点云增强参数，构造数据增强变换矩阵
        aug_transform = torch.zeros((input_dict['img_inputs'][1].shape[0],4,4)).float() # (6, 4, 4)
        if 'pcd_rotation' in input_dict:
            # 旋转矩阵变转置是因为旋转点云和box的时计算的是右乘矩阵 points[:, :3] @ rot_mat_T
            # 这里要左乘，所以需要变转置
        ...
        new_transform = aug_transform.matmul(transform) # 构造新的变换矩阵(左乘)
        input_dict['img_inputs'][1][...] = new_transform[:,:3,:3] # (6, 3, 3)
        input_dict['img_inputs'][2][...] = new_transform[:,:3,-1] # (6, 3)
