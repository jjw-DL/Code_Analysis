...
class PatchMerging(BaseModule):
    ...
    def __init__(self,
        ...
        self.sampler = nn.Unfold(
            kernel_size=stride, dilation=1, padding=0, stride=stride) # (2, 2), (1, 1), 0和(2, 2)

        sample_dim = stride**2 * in_channels # 2 * 2 * 96 = 384
    ...
    def forward(self, x, hw_shape):
        ...
        x = self.sampler(x)  # B, 4*C, H/2*W/2 --> (12, 384, 2816) : 2 * 2 * 96 = 384
        ...
        return x, down_hw_shape # (12, 2816, 192) 和 (32, 88)
    ...
    def forward(self, query, hw_shape):
        ...
        if self.shift_size > 0:
            ...
            img_mask = torch.zeros((1, H_pad, W_pad, 1),
                                   device=query.device)  # (1, 70, 182, 1)
    ...
    def __init__(self,
        ...
        for i in range(depth):
            block = SwinBlock(
                ...
                attn_drop_rate=attn_drop_rate, # 0
                drop_path_rate=drop_path_rate[i], # 0
                act_cfg=act_cfg, # GELU
                norm_cfg=norm_cfg, # LN
                init_cfg=None) # None
            self.blocks.append(block) # 将构建后的block加入ModuleList

        self.downsample = downsample # (sampler, norm, reduction)
...
class SwinTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        ]  # stochastic depth decay rule  (0, 0.2, 12)
        ...
        for i in range(num_layers): # 4
            ...
            stage = SwinBlockSequence(
                ...
                drop_path_rate=dpr[:depths[i]], # 0.2
                downsample=downsample, # (SNR)
                act_cfg=act_cfg, # GELU
                norm_cfg=norm_cfg, # LN
                init_cfg=None, # None
    ...
    def forward(self, x):
        x = self.patch_embed(x) # (12, 11264, 96)和（64, 176）
        ...
        for i, stage in enumerate(self.stages):
            # 下采样后的x和下采样前的x eg: (12, 704, 384)和[16, 44]和[12, 2861, 192]和[32, 88] <-- stage1
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape)
            if i in self.out_indices: # (2, 3,)
                norm_layer = getattr(self, f'norm{i}')
                out = norm_layer(out)
                out = out.view(-1, *out_hw_shape,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous() # (12, 32, 88, 192)-->(12, 192, 32, 88)
                outs.append(out) # (12, 384, 16, 44)和(12, 768, 8, 22)
            elif self.output_missing_index_as_none:
                outs.append(None)
        return outs # (12, 384, 16, 44)和(12, 768, 8, 22)
