...
class ResNetForBEVDet(nn.Module):
    def __init__(self, numC_input, num_layer=[2,2,2], num_channels=None, stride=[2,2,2],
        ...
        num_channels = [numC_input*2**(i+1) for i in range(len(num_layer))] \
            if num_channels is None else num_channels # (128, 256, 512)
        self.backbone_output_ids = range(len(num_layer)) \
            if backbone_output_ids is None else backbone_output_ids # (0, 1, 2)
        layers = []
        if block_type == 'BottleNeck':
            ...
            curr_numC = numC_input # 64
            for i in range(len(num_layer)): # [0, 1, 2]
                layer=[BasicBlock(curr_numC, num_channels[i], stride=stride[i],
                                  downsample=nn.Conv2d(curr_numC,num_channels[i],3,stride[i],1),
                                  norm_cfg=norm_cfg)] # 每个layer的第一层要downsample
                curr_numC = num_channels[i]
                layer.extend([BasicBlock(curr_numC, curr_numC, norm_cfg=norm_cfg) for _ in range(num_layer[i]-1)]) # 重复1次
                layers.append(nn.Sequential(*layer))
        else:
            assert False
    ...
    def forward(self, x):
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if lid in self.backbone_output_ids:
                feats.append(x_tmp)
        return feats # [(8, 128, 64, 64), (8, 256, 32, 32), (8, 512, 16, 16)]