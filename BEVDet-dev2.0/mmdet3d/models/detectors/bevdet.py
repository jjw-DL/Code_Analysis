...
class BEVDet(CenterPoint):
    ...
    def extract_img_feat(self, img, img_metas, **kwargs):
        ...
        x, depth = self.img_view_transformer([x] + img[1:7]) # eg:(8, 80, 128, 128)和(48, 59, 16, 44)
...
class BEVDetTRT(BEVDet):
    ...
    def result_deserialize(self, outs):
        ...
        for head_id in range(len(outs) // 6): # 6个task和6个head，会产生36个输出
            outs_head = [dict()]
            # 按照head重新整理
    ...
    def get_bev_pool_input(self, input):
        coor = self.img_view_transformer.get_lidar_coor(*input[1:7]) # eg: (1, 6, 59, 16, 44, 3)
