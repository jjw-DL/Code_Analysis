...
class LSSViewTransformer(BaseModule):
    ...
    def voxel_pooling_v2(self, coor, depth, feat):
        ranks_bev, ranks_depth, ranks_feat, \
            interval_starts, interval_lengths = \
            self.voxel_pooling_prepare_v2(coor) # eg: (1508751,)和(109981,)
        if ranks_feat is None:
            ...
            return dummy
        feat = feat.permute(0, 1, 3, 4, 2) # (8, 6, 16, 44, 80)
        bev_feat_shape = (depth.shape[0], int(self.grid_size[2]),
                          int(self.grid_size[1]), int(self.grid_size[0]),
                          feat.shape[-1])  # (B, Z, Y, X, C)-->(8, 1, 128, 128, 80)
        bev_feat = bev_pool_v2(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths) # eg:(8, 80, 1, 128, 128)
        # collapse Z
        bev_feat = torch.cat(bev_feat.unbind(dim=2), 1) # (8, 80, 128, 128)
        return bev_feat # (8, 80, 128, 128)
    ...
    def voxel_pooling_prepare_v2(self, coor):
        ...
        """
        B, N, D, H, W, _ = coor.shape # 1, 6, 59, 16, 44, 3
        num_points = B * N * D * H * W # 249216
        # record the index of selected points for acceleration purpose
        ranks_depth = torch.range(
            0, num_points - 1, dtype=torch.int, device=coor.device) # (249216,)
        ranks_feat = torch.range(
            0, num_points // D - 1, dtype=torch.int, device=coor.device) # (4224,) 一个像素点产生D个不同深度的空间点
        ranks_feat = ranks_feat.reshape(B, N, 1, H, W) # (1, 6, 1, 16, 44)
        # 多维数据拉成一维，batch维度要注意
        ranks_feat = ranks_feat.expand(B, N, D, H, W).flatten() # (1, 6, 59, 16, 44)-->(249216,) 沿着深度维度复制D次
        # convert coordinate into the voxel space 由真实点云坐标转换为voxel坐标
        coor = ((coor - self.grid_lower_bound.to(coor)) /
                self.grid_interval.to(coor)) # (coor - [-51, -51, -5]) / [0, 0, 8]-->(1, 6, 59, 16, 44, 3)
        coor = coor.long().view(num_points, 3) # (249216, 3) 转换long型作为index
        batch_idx = torch.range(0, B - 1).reshape(B, 1). \
            expand(B, num_points // B).reshape(num_points, 1).to(coor) # (1, 1)-->(1, 249216)-->(249216, 1)
        coor = torch.cat((coor, batch_idx), 1) # (249216, 4)

        # filter out points that are outside box 过滤预定义网格之外的点 self.grid_size:[128, 128, 1]
        ...
        coor, ranks_depth, ranks_feat = \
            coor[kept], ranks_depth[kept], ranks_feat[kept] # (179535, 4), (179535,), (179535,)
        # get tensors from the same voxel next to each other 将三维坐标拉成一维
        ranks_bev = coor[:, 3] * (
            self.grid_size[2] * self.grid_size[1] * self.grid_size[0]) # batch维度
        ranks_bev += coor[:, 2] * (self.grid_size[1] * self.grid_size[0]) # z维度
        ranks_bev += coor[:, 1] * self.grid_size[0] + coor[:, 0] # y维度 + x维度
        order = ranks_bev.argsort() # 排序索引 (179535,)
        ranks_bev, ranks_depth, ranks_feat = \
            ranks_bev[order], ranks_depth[order], ranks_feat[order] # 按照空间索引从新排序-->bev网格，深度，特征index (179535,)

        kept = torch.ones(
            ranks_bev.shape[0], device=ranks_bev.device, dtype=torch.bool) # 初始化保留标记 (179535,)
        kept[1:] = ranks_bev[1:] != ranks_bev[:-1] # 经典操作，错位对比，计算保留索引（第一个点一定保留）
        interval_starts = torch.where(kept)[0].int() # 计算voxel interval的起始点-->eg:（11404,）
        if len(interval_starts) == 0:
            return None, None, None, None, None
        interval_lengths = torch.zeros_like(interval_starts) # eg:(11404,)
        interval_lengths[:-1] = interval_starts[1:] - interval_starts[:-1] # 计算interval的长度 eg:(11403)
        interval_lengths[-1] = ranks_bev.shape[0] - interval_starts[-1] # 终点-起点
    ...
    def forward(self, input):
        ...
        """
        x = input[0] # (8, 6, 512, 16, 44)
        B, N, C, H, W = x.shape # 8, 6, 512, 16, 44
        x = x.view(B * N, C, H, W) # (48, 512, 16, 44)
        x = self.depth_net(x) # (48, 139, 16, 44)

        depth_digit = x[:, :self.D, ...] # (48, 59, 16, 44)
        tran_feat = x[:, self.D:self.D + self.out_channels, ...] # (48, 80, 16, 44)
        depth = depth_digit.softmax(dim=1) # (48, 59, 16, 44)
