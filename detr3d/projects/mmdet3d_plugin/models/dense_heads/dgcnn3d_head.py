...
class DGCNN3DHead(DETRHead):
    ...
    def __init__(self,
        ...
        if self.as_two_stage:
            transformer['as_two_stage'] = self.as_two_stage # False
        ...
        self.voxel_size = self.bbox_coder.voxel_size # [0.1, 0.1, 0.2]

        self.bev_shape = (int((self.pc_range[3] - self.pc_range[0]) / self.voxel_size[0]), 
                          int((self.pc_range[4] - self.pc_range[1]) / self.voxel_size[1])) # (1024, 1024)
    ...
    def _init_layers(self):
        ...
        if self.with_box_refine:
            self.cls_branches = _get_clones(fc_cls, num_pred) # 克隆6次
            self.reg_branches = _get_clones(reg_branch, num_pred) # 克隆6次
        else:
    ...
    def forward(self, mlvl_feats):
        ...
        input_img_h, input_img_w = self.bev_shape # 1024, 1024
        img_masks = mlvl_feats[0].new_zeros(
            (batch_size, input_img_h, input_img_w)) # (2, 1024, 1024)
        ...
        for feat in mlvl_feats:
            ...
            mlvl_positional_encodings.append(
                self.positional_encoding(mlvl_masks[-1])) # 根据mask生成位置编码
