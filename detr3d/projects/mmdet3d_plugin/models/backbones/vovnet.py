...
def conv3x3(in_channels, out_channels, module_name, postfix, stride=1, groups=1, kernel_size=3, padding=1):
    ...
    return [
        (
            f"{module_name}_{postfix}/conv", # eg:stem_1/conv
            nn.Conv2d(
                in_channels,
                out_channels,
                kernel_size=kernel_size,
...
class _OSA_module(nn.Module):
    def __init__(
        ...
        self.depthwise = depthwise # False
        self.isReduced = False # False
        self.layers = nn.ModuleList()
        in_channel = in_ch # eg: 128
        ...
        for i in range(layer_per_block): # 5
            ...
            in_channel = stage_ch # eg:128

        # feature aggregation
        in_channel = in_ch + layer_per_block * stage_ch # eg:768
        self.concat = nn.Sequential(OrderedDict(conv1x1(in_channel, concat_ch, module_name, "concat"))) # [256, 512, 768, 1024]
...
class _OSA_stage(nn.Sequential):
    def __init__(
        ...
        if not stage_num == 2: # 第2个stage不需要池化
            self.add_module("Pooling", nn.MaxPool2d(kernel_size=3, stride=2, ceil_mode=True))

        if block_per_stage != 1: # 只有一个block的stage需要SE
            SE = False
        module_name = f"OSA{stage_num}_1" # eg:OSA2_1
        self.add_module(
            module_name, _OSA_module(in_ch, stage_ch, concat_ch, layer_per_block, module_name, SE, depthwise=depthwise)
        )
        for i in range(block_per_stage - 1): # [1, 3, 9, 3]
            if i != block_per_stage - 2:  # last block
                SE = False
            module_name = f"OSA{stage_num}_{i + 2}" # 'OSA2_2'...'OSA2_5'
            self.add_module(
                module_name,
                _OSA_module(
                    concat_ch, # [256, 512, 768, 1024]
                    stage_ch, # [128, 160, 192, 224]
                    concat_ch, # [256, 512, 768, 1024]
                    layer_per_block, # [1, 3, 9, 3]
                    module_name, # 'OSA2_2'...'OSA2_5'
                    SE,# True
                    identity=True, # True
...
class VoVNet(BaseModule):
    def __init__(self, spec_name, input_ch=3, out_features=None, 
        ...
        stage_specs = _STAGE_SPECS[spec_name] # VoVNet99_eSE
        ...
        self._out_features = out_features # ['stage2', 'stage3', 'stage4', 'stage5']

        # Stem module
        conv_type = dw_conv3x3 if depthwise else conv3x3 # conv3x3
        stem = conv3x3(input_ch, stem_ch[0], "stem", "1", 2) # 3-->64 stem_1/conv
        stem += conv_type(stem_ch[0], stem_ch[1], "stem", "2", 1) # 64-->64 stem_2/conv
        stem += conv_type(stem_ch[1], stem_ch[2], "stem", "3", 2) # 64-->128 stem_3/conv x4下采样
        self.add_module("stem", nn.Sequential((OrderedDict(stem))))
        current_stirde = 4
        self._out_feature_strides = {"stem": current_stirde, "stage2": current_stirde} # 4
        ...
        for i in range(4):  # num_stages
            ...
            self.add_module(
                ...
                _OSA_stage(
                    ...
                    config_stage_ch[i], # [128, 160, 192, 224]
                    config_concat_ch[i], # [256, 512, 768, 1024]
                    block_per_stage[i], # [1, 3, 9, 3]
                    layer_per_block, # 5
                    i + 2, # 2 ... 5
                    SE, # True
                    depthwise, # False
            ...
            self._out_feature_channels[name] = config_concat_ch[i] # [256, 512, 768, 1024]
            if not i == 0:
                self._out_feature_strides[name] = current_stirde = int(current_stirde * 2) # [8, 16, 32]
    ...
    def forward(self, x):
        ...
        return outputs # (6, 128, 232, 400)和(6, 512, 116, 200)和(6, 768, 58, 100)和(6, 1024, 29, 50)
