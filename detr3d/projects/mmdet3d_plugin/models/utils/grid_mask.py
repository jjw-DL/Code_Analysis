...
class GridMask(nn.Module):
    ...
    def forward(self, x):
        if np.random.rand() > self.prob or not self.training:
            return x
        n,c,h,w = x.size() # (6, 3, 928, 1600)
        x = x.view(-1,h,w) # (18, 928, 1600)
        hh = int(1.5*h) # 1392
        ww = int(1.5*w) # 2400
        d = np.random.randint(2, h) # eg:825
        self.l = min(max(int(d*self.ratio+0.5),1),d-1) # 413
        ...
        mask = mask[(hh-h)//2:(hh-h)//2+h, (ww-w)//2:(ww-w)//2+w] # (928, 1600)

        mask = torch.from_numpy(mask).float().cuda()
        if self.mode == 1:
            mask = 1-mask # (928, 1600)
        mask = mask.expand_as(x) # (18, 928, 1600)
        if self.offset:
            offset = torch.from_numpy(2 * (np.random.rand(h,w) - 0.5)).float().cuda()
            x = x * mask + offset * (1 - mask)
        else:
            x = x * mask # (18, 928, 1600)

        return x.view(n,c,h,w) # (6, 3, 928, 1600)