...
class PointToMultiViewDepth(object):
    def __init__(self, downsample=16, num_depth=59, object=False):
        self.downsample = downsample # 16
        self.num_depth = num_depth # 59

    def points2depthmap(self, points, height, width, canvas=None):
        height, width = height//self.downsample, width//self.downsample # 计算下采样后特征图的宽和高 (16, 44)
        depth_map = torch.zeros((height,width), dtype=torch.float32) # (16, 44)
        coor = torch.round(points[:,:2]/self.downsample) # 计算点云投影的图像坐标 (34720, 2) 取整
        depth = points[:,2] # 点云的对应深度 (34720, 1)
        kept1 = (coor[:, 0] >= 0) & (coor[:, 0] < width) \
               & (coor[:, 1] >= 0) & (coor[:, 1] < height) \
                & (depth < 60) & (depth >= 1.) # 投影不在当前图像上的或者深度超过60的过滤掉
        coor, depth = coor[kept1], depth[kept1] # eg:(2748, 2)
        ranks = coor[:, 0] + coor[:, 1] * width # 将坐标拉直 (2748,)
        sort = (ranks+depth/100.).argsort()
        coor, depth, ranks = coor[sort], depth[sort], ranks[sort]

        kept2 = torch.ones(coor.shape[0], device=coor.device, dtype=torch.bool)
        kept2[1:] = (ranks[1:] != ranks[:-1]) # 相同坐标只保留一个点的深度 去重
        coor, depth = coor[kept2], depth[kept2] # eg:(654, 2)
        coor = coor.to(torch.long)
        depth_map[coor[:,1],coor[:,0]] = depth # (16, 44)
        return depth_map

    def __call__(self, results):
        points_lidar = results['points'] # 读取点云(34720, 5)
        # imgs:(6, 3, 256, 704)
        # rots:(6, 3, 3)
        # trans:(6, 3)
        # intrins:(6, 3, 3)
        # post_rots:(6, 3, 3)
        # post_trans:(6, 3)
        imgs, rots, trans, intrins, post_rots, post_trans = results['img_inputs'] # 读取图片，内外参和对应的后处理变换矩阵
        depth_map_list = []
        # 逐图片处理
        for cid in range(rots.shape[0]):
            combine = rots[cid].matmul(torch.inverse(intrins[cid])) # img2lidar的旋转矩阵 (3, 3)
            combine_inv = torch.inverse(combine) # lidar2img的旋转矩阵 (3, 3)
            # 将lidar点转换到img上，右乘，trans[cid:cid+1,:]表示cam2lidar的平移，lidar2cam-->cam2img
            points_img = (points_lidar.tensor[:,:3] - trans[cid:cid+1,:]).matmul(combine_inv.T) # (34720, 3)
            points_img = torch.cat([points_img[:,:2]/points_img[:,2:3],
                                   points_img[:,2:3]], 1) # 深度归一化 (34720, 3)
            # 将图像坐标乘后处理变换矩阵-->range img
            points_img = points_img.matmul(post_rots[cid].T)+post_trans[cid:cid+1,:] # (34720, 3)
            depth_map = self.points2depthmap(points_img, imgs.shape[2], imgs.shape[3])
            depth_map_list.append(depth_map)
        depth_map = torch.stack(depth_map_list) # (6, 16, 44)
