...
class BEVDepth_Base():
    ...
    def get_depth_loss(self, depth_gt, depth):
        B, N, H, W = depth_gt.shape # 8, 6, 16, 44
        # (8, 6, 16, 44)-->(8, 6, 1, 16, 44)-->(8, 6, 59, 16, 44): loss的权重
        loss_weight = (~(depth_gt == 0)).reshape(B, N, 1, H, W).expand(B, N,
                                                                       self.img_view_transformer.D,
                                                                       H, W)
        # self.img_view_transformer.grid_config['dbound'][0] : 1
        # self.img_view_transformer.D : 59
        # 对深度进行截断
        depth_gt = torch.clip(torch.floor(depth_gt) -
                              self.img_view_transformer.grid_config['dbound'][
                                  0],
                              0, self.img_view_transformer.D).to(torch.long) # (8, 6, 16, 44)
        
        depth_gt_logit = F.one_hot(depth_gt.reshape(-1),
                                   num_classes=self.img_view_transformer.D) # (33792, 59)
        # (33792, 59)-->(8, 6, 16, 44, 59)-->(8, 6, 59, 16, 44)
        depth_gt_logit = depth_gt_logit.reshape(B, N, H, W,
                                                self.img_view_transformer.D).permute(
            0, 1, 4, 2, 3).to(torch.float32)
        # 对预测深度取sigmoid,然后reshpe-->(8, 6, 59, 16, 44)
        depth = depth.sigmoid().view(B, N, self.img_view_transformer.D, H, W)

        loss_depth = F.binary_cross_entropy(depth, depth_gt_logit,
                                            weight=loss_weight) # 二分类交叉熵计算深度预测损失
        # self.img_view_transformer.loss_depth_weight:100
        loss_depth = self.img_view_transformer.loss_depth_weight * loss_depth # 乘深度权重
        return loss_depth # eg:60.4532
...
class BEVDepth(BEVDepth_Base, BEVDet):
    ...
    def forward_train(self,
        ...
        img_feats, pts_feats, depth = self.extract_feat(
            points, img=img_inputs, img_metas=img_metas) # (8, 256, 128, 128), None, (48, 59, 16, 44)
        assert self.with_pts_bbox

        depth_gt = img_inputs[-1] # (8, 6, 16, 44)
        # 计算深度的loss！！！
