...
class SingleTeacherDistiller(BaseDistiller):
    ...
    def __init__(self,
        ...
        self.teacher_trainable = teacher_trainable # False
        self.teacher_norm_eval = teacher_norm_eval # True
        self.teacher = self.build_teacher(teacher)

        self.components = components # {'student_module', 'teacher_module', 'losses'}
        ...
        for i, component in enumerate(self.components):
            student_module_name = component['student_module'] # decode_head.conv_seg
            teacher_module_name = component['teacher_module'] # decode_head.conv_seg
            ...
            for loss in component.losses:
                loss_cfg = loss.copy()
                loss_name = loss_cfg.pop('name') # loss_cwd_logits
    ...
    def prepare_from_student(self, student):
        ...
        # 1.Record the mapping relationship between student's modules and module names.
        self.student_module2name = {}
        for name, module in student.model.named_modules():
            self.student_module2name[module] = name
        self.student_name2module = dict(student.model.named_modules()) # named_modules返回的就是dict

        # 2.Record the mapping relationship between teacher's modules and module names.
        ...
        # 3.Register forward hooks for modules that need to participate in loss calculation.
    ...
    def teacher_forward_output_hook(self, module, inputs, outputs):
        """Save the module's forward output.
           torch的hook参数固定, module, inputs和outputs
        Args:
    ...
    def compute_distill_loss(self, data=None):
        ...
        for i, component in enumerate(self.components):
            ...
            student_module_name = component['student_module'] # decode_head.conv_seg
            student_outputs = self.student_outputs[student_module_name] # [(2, 19, 64, 128)]

            # Align student output's channels with teacher.
            align_module_name = f'component_{i}' # component_0
            ...
            teacher_module_name = component['teacher_module'] # decode_head.conv_seg
            teacher_outputs = self.get_teacher_outputs(teacher_module_name) # [(2, 19, 64, 128)]
            ...
            for out_idx, (s_out, t_out) in enumerate(
                ...
                for loss in component.losses:
                    loss_module = self.losses[loss.name] # ChannelWiseDivergence
                    loss_name = f'{loss.name}.{out_idx}' # loss_cwd_logits.0
                    # TODO ugly implementation.
                    # Pass the gt_label to loss function.
                    # Only used by WSLD.
                    loss_module.current_data = data # 记录输入数据
                    losses[loss_name] = loss_module(s_out, t_out) # 计算损失
