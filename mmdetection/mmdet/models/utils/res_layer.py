        ...
        if downsample_first:
            layers.append(
                block(
                    ...
                    planes=planes, # 64
                    stride=stride,
                    downsample=downsample, # 第一层存在downsample
                    conv_cfg=conv_cfg,
                    norm_cfg=norm_cfg,
                    **kwargs))
            inplanes = planes * block.expansion # 256
            for _ in range(1, num_blocks): # 从1开始
                layers.append(
                    block(
                        inplanes=inplanes, # 256
                        planes=planes, # 64
                        stride=1,
                        conv_cfg=conv_cfg,
                        norm_cfg=norm_cfg,
                        **kwargs)) # eg: 64-->64-->64-->256 (第一block比较特殊inplanes和planes相同)
