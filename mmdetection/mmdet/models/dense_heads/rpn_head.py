...
class RPNHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        if self.num_convs > 1:
            ...
            self.rpn_conv = nn.Conv2d(
                self.in_channels, self.feat_channels, 3, padding=1) # 256-->256
    ...
    def forward_single(self, x):
        ...
        x = self.rpn_conv(x) # (2, 256, 272, 200)
        x = F.relu(x, inplace=True)
        rpn_cls_score = self.rpn_cls(x) # (2, 3, 272, 200)
        rpn_bbox_pred = self.rpn_reg(x) # (2, 12, 272, 200)
    ...
    def _bbox_post_process(self, mlvl_scores, mlvl_bboxes, mlvl_valid_anchors,
        ...
        Returns:
            Tensor: Labeled boxes in shape (n, 5), where the first 4 columns
                ...
                5-th column is a score between 0 and 1.
        """
        # 将5个特征图上的score, anchor和rpn_bbox_pred拼接到一起
        scores = torch.cat(mlvl_scores)
        anchors = torch.cat(mlvl_valid_anchors)
        rpn_bbox_pred = torch.cat(mlvl_bboxes)
        # 对所有采集的anchor进行解码还原到原始图像
