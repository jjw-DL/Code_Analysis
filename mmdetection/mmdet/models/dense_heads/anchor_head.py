...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        if self.train_cfg:
            self.assigner = build_assigner(self.train_cfg.assigner) # 'MaxIoUAssigner'
            if hasattr(self.train_cfg,
                ...
                self.sampling = False
                sampler_cfg = dict(type='PseudoSampler')
            self.sampler = build_sampler(sampler_cfg, context=self) # 'RandomSampler'
        self.fp16_enabled = False

        self.prior_generator = build_prior_generator(anchor_generator) # 'AnchorGenerator'

        # Usually the numbers of anchors for each level are the same
        # except SSD detectors. So it is an int in the most dense
        # heads but a list of int in SSDHead
        self.num_base_priors = self.prior_generator.num_base_priors[0] # 3
        self._init_layers()
    ...
    def get_anchors(self, featmap_sizes, img_metas, device='cuda'):
        ...
        Returns:
            tuple:
                anchor_list (list[5 Tensor]): Anchors of each image.
                valid_flag_list (list[5 Tensor]): Valid flags of each image.
        """
        ...
        multi_level_anchors = self.prior_generator.grid_priors(
            featmap_sizes, device=device) # List[5] (N, 4)
        ...
        return anchor_list, valid_flag_list # List[5个Tensor(N, 4)]和List[5个Tensor(N, 1)]
    ...
    def _get_targets_single(self,
        ...
        inside_flags = anchor_inside_flags(flat_anchors, valid_flags,
                                           ...
                                           self.train_cfg.allowed_border) # eg:(217413,)
        if not inside_flags.any():
            return (None, ) * 7
        
        # 2.assign gt and sample anchors
        anchors = flat_anchors[inside_flags, :] # 提取所有有效anchor eg:(211038, 4)
        ...
        bbox_weights = torch.zeros_like(anchors) # 全0初始化bbox的权重，通过权重避免计算neg box
        # -------------------------------
        # 针对rpn是全1初始化，因为只有一个类
        # -------------------------------
        ...
        label_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float) # label的权重全部初始化为0 eg:(211038,)
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox: # 如果不是回归解码的bbox，则要进行bbox的编码
                pos_bbox_targets = self.bbox_coder.encode(
                    sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes) # 对正例进行box编码 eg:(8, 4)
            else:
            ...
            if gt_labels is None:
                ...
                labels[pos_inds] = 0 # 如果没有gt，则将正例的的lable设置为0, RPN只有两类（前景和背景）
            else:
    ...
    def get_targets(self,
        ...
        num_level_anchors = [anchors.size(0) for anchors in anchor_list[0]] # 获取每个尺度的anchor的数量 List[163200, 40800, 10200, 2550, 663]
        ...
        for i in range(num_imgs):
            ...
            concat_anchor_list.append(torch.cat(anchor_list[i])) # eg:(217413, 4)
        ...
        # 根据每层anchor的数量将label，bbox target分割到各个level
    ...
    def loss(self,
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        featmap_sizes = [featmap.size()[-2:] for featmap in cls_scores] # [(272, 200), (156, 100), (68, 50), (34, 25), (17, 13)]
