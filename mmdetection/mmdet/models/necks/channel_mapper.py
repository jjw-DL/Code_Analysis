...
class ChannelMapper(BaseModule):
    ...
    def __init__(self,
                 ...
                 kernel_size=3, # 1
                 conv_cfg=None,
                 norm_cfg=None, # dict(type='GN', num_groups=32)
        ...
        for in_channel in in_channels:
            self.convs.append(
                ConvModule(
                    ...
                    conv_cfg=conv_cfg, # None-->conv2d
                    norm_cfg=norm_cfg, # dict(type='GN', num_groups=32)
                    act_cfg=act_cfg)) # dict(type='ReLU')
        if num_outs > len(in_channels):
            self.extra_convs = nn.ModuleList()
            for i in range(len(in_channels), num_outs):
                if i == len(in_channels):
                    in_channel = in_channels[-1] # 2048
                else:
