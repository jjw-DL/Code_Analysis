...
class SingleRoIExtractor(BaseRoIExtractor):
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        ...
        for i in range(num_levels):
            mask = target_lvls == i # 计算该特征层的roi mask
            if torch.onnx.is_in_onnx_export():
                ...
                continue
            inds = mask.nonzero(as_tuple=False).squeeze(1) # 计算该特征层roi的索引
            if inds.numel() > 0:
                rois_ = rois[inds] # 提取该特征层的roi
                roi_feats_t = self.roi_layers[i](feats[i], rois_) # ROI Align进行特征提取
