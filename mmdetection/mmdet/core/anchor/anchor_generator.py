...
class AnchorGenerator:
    ...
    def __init__(self,
        ...
        self.strides = [_pair(stride) for stride in strides] # [(4,4), (8, 8), (16, 16), (32, 32), (64, 64)]
        self.base_sizes = [min(stride) for stride in self.strides
                           ] if base_sizes is None else base_sizes # base_size根据stride生成 [4, 8, 16, 32, 64]
        ...
        if scales is not None:
            self.scales = torch.Tensor(scales) # 在Faster RCNN中只设置一个尺度
        elif octave_base_scale is not None and scales_per_octave is not None:
            octave_scales = np.array(
                [2**(i / scales_per_octave) for i in range(scales_per_octave)])
            scales = octave_scales * octave_base_scale
            self.scales = torch.Tensor(scales) # 在RetinaNet中设置了多个尺度
        else:
            raise ValueError('Either scales or octave_base_scale with '
                             'scales_per_octave should be set')

        self.octave_base_scale = octave_base_scale # None
        self.scales_per_octave = scales_per_octave # None
    ...
    def gen_base_anchors(self):
        ...
        # base_size : Basic size of an anchor 根据stride生成
    ...
    def grid_priors(self, featmap_sizes, dtype=torch.float32, device='cuda'):
        ...
        for i in range(self.num_levels):
            anchors = self.single_level_grid_priors(
                featmap_sizes[i], level_idx=i, dtype=dtype, device=device) # 生成该level下的anchor
            multi_level_anchors.append(anchors) # 将各level的anchor加入anchor list
    ...
    def single_level_grid_priors(self,
        ...
        base_anchors = self.base_anchors[level_idx].to(device).to(dtype) # 根据level_idx选取base_anchors
        feat_h, feat_w = featmap_size
        stride_w, stride_h = self.strides[level_idx] # 根据level_idx获取stride
        # First create Range with the default dtype, than convert to
        # target `dtype` for onnx exporting.
        shift_x = torch.arange(0, feat_w, device=device).to(dtype) * stride_w # 还原到真实图片位置
        shift_y = torch.arange(0, feat_h, device=device).to(dtype) * stride_h

        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y) # (N*M,) eg:(54400)
        shifts = torch.stack([shift_xx, shift_yy, shift_xx, shift_yy], dim=-1) # (N*M, 4)
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        multi_level_flags = []
        for i in range(self.num_levels):
            anchor_stride = self.strides[i] # 获取stride
            feat_h, feat_w = featmap_sizes[i] # 获取特征图尺寸
            h, w = pad_shape[:2] # 获取pad前图片尺寸
            valid_feat_h = min(int(np.ceil(h / anchor_stride[1])), feat_h) # 计算有效的特征图高
            valid_feat_w = min(int(np.ceil(w / anchor_stride[0])), feat_w) # 计算有效的特征图宽
    ...
    def single_level_valid_flags(self,
        ...
        Returns:
            torch.Tensor: The valid flags of each anchor in a single level \
                feature map.
        """
        feat_h, feat_w = featmap_size # 特征图高宽
        valid_h, valid_w = valid_size # 有效特征图高和宽
        assert valid_h <= feat_h and valid_w <= feat_w
        valid_x = torch.zeros(feat_w, dtype=torch.bool, device=device)
        valid_y = torch.zeros(feat_h, dtype=torch.bool, device=device)
        valid_x[:valid_w] = 1 # 将有效位置赋为1, pad是只在右侧进行的（不是中心pad）
        valid_y[:valid_h] = 1
        valid_xx, valid_yy = self._meshgrid(valid_x, valid_y) # 生成网格图
        valid = valid_xx & valid_yy # 只有两者全为1的地方才有效
        # repeat()和expand() 都是复制tensor，不同的是repeat()是复制，expand是改变视图
