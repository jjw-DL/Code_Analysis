...
def multi_apply(func, *args, **kwargs):
    ...
    map_results = map(pfunc, *args) # map会将tuple内的元素逐个输入
    return tuple(map(list, zip(*map_results))) # 将返回值同一个字段的内容组成一个list，最外层套上tuple
...
def unmap(data, count, inds, fill=0):
    ...
    count)"""
    if data.dim() == 1:
        ret = data.new_full((count, ), fill) # 初始化结果
        ret[inds.type(torch.bool)] = data # 将数据赋予对应位置
    else:
        new_size = (count, ) + data.size()[1:] # 计算新的size，eg:(n, 4)
...
def filter_scores_and_topk(scores, score_thr, topk, results=None):
    ...
    scores = scores[valid_mask] # 获取分数大于分数阈值的分数
    valid_idxs = torch.nonzero(valid_mask) # 找到对应索引

    num_topk = min(topk, valid_idxs.size(0)) # 计算topk的个数，如果有效值已经小于规定topk，则直接取当前个数
    # torch.sort is actually faster than .topk (at least on GPUs)
    scores, idxs = scores.sort(descending=True) # 对分数进行降序排序
    scores = scores[:num_topk] # 取出前topk个分数
    topk_idxs = valid_idxs[idxs[:num_topk]] # 找到原始分数对应索引
    keep_idxs, labels = topk_idxs.unbind(dim=1) # 返回指定维度切片后的元组

    filtered_results = None
    if results is not None:
        if isinstance(results, dict):
            # 将预测的box和对应的anchor加入filtered_results中
