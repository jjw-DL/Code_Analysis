...
def main(nusc, scene_names, root_path, calib_folder, mode):
    ...
    for scene_index, scene_info in enumerate(nusc.scene):
        ...
        frame_index = 0
        calib_data = dict() # 初始化calib_data的list
        while True:
            if mode == '2hz':
                ...
                calib_token = lidar_data['calibrated_sensor_token'] # 在lidar data中获取calib_token
                calib_pose = nusc.get('calibrated_sensor', calib_token) # 根据calib_token获取标定信息
            elif mode == '20hz':
                frame_data = nusc.get('sample_data', cur_sample_token) # 这里已经是lidar data
                calib_token = frame_data['calibrated_sensor_token'] # 获取calib_token
                calib_pose = nusc.get('calibrated_sensor', calib_token) # 根据calib_token获取标定信息
