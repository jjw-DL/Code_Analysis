...
def giou3d(box_a: BBox, box_b: BBox):
    boxa_corners = np.array(BBox.box2corners2d(box_a))[:, :2] # (4, 2) 底部4个角点坐标
    boxb_corners = np.array(BBox.box2corners2d(box_b))[:, :2]
    reca, recb = Polygon(boxa_corners), Polygon(boxb_corners) # 创建polygon对象
    ha, hb = box_a.h, box_b.h
    za, zb = box_a.z, box_b.z
    overlap_height = max(0, min((za + ha / 2) - (zb - hb / 2), (zb + hb / 2) - (za - ha / 2))) # 重叠高度
    union_height = max((za + ha / 2) - (zb - hb / 2), (zb + hb / 2) - (za - ha / 2)) # 联合高度
    
    # compute intersection and union
    I = reca.intersection(recb).area * overlap_height # 计算交集的体积
    U = box_a.w * box_a.l * ha + box_b.w * box_b.l * hb - I # 计算并集的volume

    # compute the convex area
    all_corners = np.vstack((boxa_corners, boxb_corners)) # 获取全部角点
    C = ConvexHull(all_corners) # 构造凸包
    convex_corners = all_corners[C.vertices] # 计算凸包角点
    convex_area = PolyArea2D(convex_corners) # 计算凸包面积
    C = convex_area * union_height # 计算外包体的volume

    # compute giou
    giou = I / U - (C - U) / C # 计算Giou
    return giou
