...
def nms(dets, inst_types, threshold_low=0.1, threshold_high=1.0, threshold_yaw=0.3):
    ...
    """
    dets_coarse_filter = BBoxCoarseFilter(grid_size=100, scaler=100) # 创建bbox的粗略过滤器
    dets_coarse_filter.bboxes2dict(dets)
    scores = np.asarray([det.s for det in dets]) # 获取分数
    yaws = np.asarray([det.o for det in dets]) # 获取yaws
    order = np.argsort(scores)[::-1] # 按照分数逆序排序
    
    result_indexes = list()
    result_types = list()
    # 开始NMS
    while order.size > 0:
        index = order[0] # 获取保留bbox索引

        if weird_bbox(dets[index]): # 如果该bbox是不合法bbox，则删除
            order = order[1:]
            continue

        # locate the related bboxes
        filter_indexes = dets_coarse_filter.related_bboxes(dets[index]) # 获取相关的bbox索引
        in_mask = np.isin(order, filter_indexes) # 提取在order中的box
        related_idxes = order[in_mask]
        related_idxes = np.asarray([i for i in related_idxes if inst_types[i] == inst_types[index]]) # 提取该类别的bbox

        # compute the ious 计算iou
        bbox_num = len(related_idxes)
        ious = np.zeros(bbox_num)
        for i, idx in enumerate(related_idxes):
            ious[i] = utils.iou3d(dets[index], dets[idx])[1] # 返回的是iou2d和iou3d，index为1，表示去iou3d
        ...
        if len(order_vote) >= 2:
            ...
            result_indexes.append(index) # 保留该bbox的索引
            result_types.append(inst_types[index]) # 加入该bbox的类别
