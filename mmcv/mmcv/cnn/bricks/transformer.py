...
class MultiheadAttention(BaseModule):
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        return identity + self.dropout_layer(self.proj_drop(out)) # ADD
...
class FFN(BaseModule):
    ...
    def __init__(self,
        ...
        self.embed_dims = embed_dims # 256
        self.feedforward_channels = feedforward_channels # 2048
        ...
        for _ in range(num_fcs - 1):
            layers.append(
                Sequential(
                    ...
                    nn.Dropout(ffn_drop))) # 256-->1024
            in_channels = feedforward_channels
        layers.append(Linear(feedforward_channels, embed_dims)) # 1024-->256
...
class BaseTransformerLayer(BaseModule):
    ...
    def __init__(self,
        ...
        self.operation_order = operation_order # {'self_attn', 'norm', 'ffn', 'norm'}
        ...
        for operation_name in operation_order:
            if operation_name in ['self_attn', 'cross_attn']:
                if 'batch_first' in attn_cfgs[index]:
                    ...
                    attn_cfgs[index]['batch_first'] = self.batch_first # False
        ...
        for ffn_index in range(num_ffns):
            ...
            self.ffns.append(
                build_feedforward_network(ffn_cfgs[ffn_index],
                                          dict(type='FFN'))) # 根据num_ffn层构建ffns层
        ##########################
        # 初始化Norm
        ##########################
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                query = self.attentions[attn_index](
                    ...
                    key_padding_mask=key_padding_mask, # (2, 12231)
                    **kwargs)
                attn_index += 1
                identity = query
...
class TransformerLayerSequence(BaseModule):
    ...
    def __init__(self, transformerlayers=None, num_layers=None, init_cfg=None):
        ...
        if isinstance(transformerlayers, dict):
            ...
            ] # 复制6次'BaseTransformerLayer'配置
        else:
            assert isinstance(transformerlayers, list) and \
                   len(transformerlayers) == num_layers
        self.num_layers = num_layers # 6
        self.layers = ModuleList() # 初始化ModuleList
        for i in range(num_layers):
            self.layers.append(build_transformer_layer(transformerlayers[i])) # 构建6层transformerlayer
        self.embed_dims = self.layers[0].embed_dims # 256
        self.pre_norm = self.layers[0].pre_norm # False
    ...
    def forward(self,
        ...
        for layer in self.layers:
            query = layer(
                ...
                key, # None
                value, # None
                query_pos=query_pos, # (525, 2, 256)
                key_pos=key_pos, # None
                attn_masks=attn_masks, # None
                query_key_padding_mask=query_key_padding_mask, # (2, 525)
                key_padding_mask=key_padding_mask, # (2, 525)
                **kwargs)
        return query # (525, 2, 256)
