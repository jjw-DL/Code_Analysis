...
class MetricsHost:
    ...
    def compute(
        ...
        df_map = events_to_df_map(df) # 构建DataFrameMap
    ...
    def _compute(self, df_map, name, cache, options, parent=None):
        ...
        if already is not None:
            return already
        minfo = self.metrics[name] # 选择对应的metrics
        vals = []
        # 遍历计算该metric的依赖
        for depname in minfo["deps"]:
            v = cache.get(depname, None) # 从cache中获取该dep
            if v is None: # 如果该dep在cache中没有，则计算该dep
        ...
        if _getargspec(minfo["fnc"]).defaults is None:
            ...
            return minfo["fnc"](df_map, *vals, **options) # 调用该metric的计算函数，计算该值
...
def events_to_df_map(df):
    raw = df[df.Type == "RAW"] # 提取类型为RAW的df
    noraw = df[
        (df.Type != "RAW")
        & (df.Type != "ASCEND")
        & (df.Type != "TRANSFER")
