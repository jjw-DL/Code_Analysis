...
class MOTAccumulator(object):
    ...
    def __init__(self, auto_id=False, max_switch_time=float('inf')):
        ...
        self.max_switch_time = max_switch_time # 生成跟踪转换事件的最大时间
    ...
    def reset(self):
        ...
        self._events = {field: [] for field in _EVENT_FIELDS} # 构建dict
    ...
    def update(self, oids, hids, dists, frameid=None, vf=''):
        ...
        self.dirty_events = True
        oids = np.asarray(oids) # object ids
        oids_masked = np.zeros_like(oids, dtype=np.bool) # object ids mask
        hids = np.asarray(hids) # hypothesis ids
        hids_masked = np.zeros_like(hids, dtype=np.bool) # hypothesis ids
        dists = np.atleast_2d(dists).astype(float).reshape(oids.shape[0], hids.shape[0]).copy() # distances
        ...
        eid = itertools.count() # 创建迭代器

        # 0. Record raw events 记录原始事件

        no = len(oids) # number of objects
        nh = len(hids) # number of hypothesis

        # Add a RAW event simply to ensure the frame is counted.
        # 添加一个 RAW 事件只是为了确保对帧进行计数
        ...
        # 为存在但不与任何对象重叠的对象和假设添加RAW事件
        used_i = np.unique(valid_i)
        used_j = np.unique(valid_j)
        unused_i = np.setdiff1d(np.arange(no), used_i) # np.setdiff1d找到两个数组中集合元素的差异
        ...
        if oids.size * hids.size > 0:
            ...
            for i in range(oids.shape[0]):
                ...
                hprev = self.m[oids[i]] # 获取之前的hypothesis
                j, = np.where(~hids_masked & (hids == hprev)) # 计算hids==hprev的所有有效id的索引
                if j.shape[0] == 0:
                    continue
                j = j[0] # 取第一个有效id索引

                if np.isfinite(dists[i, j]): # 如果二者距离有限
                    o = oids[i] # 获取object id
                    h = hids[j] # 获取hypothesis id
                    oids_masked[i] = True # 将mask计为true，防止其他匹配
                    hids_masked[j] = True
                    self.m[oids[i]] = hids[j] # 记录匹配关系

                    self._append_to_indices(frameid, next(eid)) # 添加事件id
                    self._append_to_events('MATCH', oids[i], hids[j], dists[i, j]) # 添加匹配事件
                    self.last_match[o] = frameid # 记录帧id
                    self.hypHistory[h] = frameid

            # 2. Try to remaining objects/hypotheses
            # 尝试处理剩余objects
            dists[oids_masked, :] = np.nan # 对于已经匹配的object的距离设置为nan
            dists[:, hids_masked] = np.nan

            rids, cids = linear_sum_assignment(dists) # 计算o和h的匹配关系
            ...
            for i, j in zip(rids, cids):
                ...
                o = oids[i] # object id
                h = hids[j] # hypothesis id
                # 如果object匹配的hypothesis与之前帧不同，并且当前帧与上一次对应的匹配差很多，超过阈值则判定switch
                is_switch = (o in self.m and
                             self.m[o] != h and
                             abs(frameid - self.last_occurrence[o]) <= self.max_switch_time)
                cat1 = 'SWITCH' if is_switch else 'MATCH' # 记录分类
                if cat1 == 'SWITCH': # 如果判定为switch了
                    if h not in self.hypHistory: # 并且这个假设不在之前的历史中
                        subcat = 'ASCEND' # 判定为Switch中的Ascned
                        self._append_to_indices(frameid, next(eid))
                        self._append_to_events(subcat, oids[i], hids[j], dists[i, j])
                # ignore the last condition temporarily 暂时忽略最后一个条件
                is_transfer = (h in self.res_m and
                               self.res_m[h] != o) # 如果h在剩余的匹配中，并且对应的匹配不是o，则判定为transfer
                # is_transfer = (h in self.res_m and
                #                self.res_m[h] != o and
                #                abs(frameid - self.last_occurrence[o]) <= self.max_switch_time)
                cat2 = 'TRANSFER' if is_transfer else 'MATCH'
                if cat2 == 'TRANSFER':
                    if o not in self.last_match: # 如果o不在上一次的匹配中
                        subcat = 'MIGRATE'
                        self._append_to_indices(frameid, next(eid))
                        self._append_to_events(subcat, oids[i], hids[j], dists[i, j])
                    self._append_to_indices(frameid, next(eid))
                    self._append_to_events(cat2, oids[i], hids[j], dists[i, j])
                # 如果cat1或着cat2不是Match，并且vf不是空，则记录输出
        ...
        # 更新当前的状态
        for o in oids:
            self.last_occurrence[o] = frameid # 记录上一次匹配的帧id
    ...
    def merge_event_dataframes(dfs, update_frame_indices=True, update_oids=True, update_hids=True, return_mappings=False):
        ...
        mapping_infos = []
        new_oid = itertools.count() # 创建新object id总的迭代器，用于数据合并映射
        new_hid = itertools.count()

        r = MOTAccumulator.new_event_dataframe() # 创建一个新的event dataframe
        # 逐个dataframe处理
        for df in dfs:
