...
def _write_obj(points, out_filename):
    ...
    for i in range(N):
        if points.shape[1] == 6:
            ...
            fout.write('v %f %f %f\n' %
                       (points[i, 0], points[i, 1], points[i, 2])) # 保存点云的xyz坐标
    fout.close()
...
def _write_oriented_bbox(scene_bbox, out_filename):
    ...
    def heading2rotmat(heading_angle):
        ...
        rotmat[0:2, 0:2] = np.array([[cosval, -sinval], [sinval, cosval]]) # 构造2维旋转矩阵
        return rotmat

    def convert_oriented_box_to_trimesh_fmt(box):
        ctr = box[:3]  # 获取bbox的中心
        lengths = box[3:6] # 获取bbox的长宽和高
        trns = np.eye(4) # 构造单位矩阵
        trns[0:3, 3] = ctr
        trns[3, 3] = 1.0
        trns[0:3, 0:3] = heading2rotmat(box[6]) # 将box的中心绕z轴旋转
        box_trimesh_fmt = trimesh.creation.box(lengths, trns) # 创建mesh形式的bbox
        return box_trimesh_fmt

    if len(scene_bbox) == 0:
        scene_bbox = np.zeros((1, 7))
    scene = trimesh.scene.Scene() # 创建mesh的Scene
    for box in scene_bbox:
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(box)) # 向Scene中逐个添加bbox

    mesh_list = trimesh.util.concatenate(scene.dump()) # 将场景concat成list
    # save to obj file
    trimesh.io.export.export_mesh(mesh_list, out_filename, file_type='obj') # 保存mesh
...
def show_result(points,
    ...
    """
    result_path = osp.join(out_dir, filename) # 拼接输出文件路径
    mmcv.mkdir_or_exist(result_path) # 创建输出文件夹

    if show:
        from .open3d_vis import Visualizer

        vis = Visualizer(points) # 构造Visualizer对象
        if pred_bboxes is not None:
            vis.add_bboxes(bbox3d=pred_bboxes) # 添加预测的3D bbox
        if gt_bboxes is not None:
            vis.add_bboxes(bbox3d=gt_bboxes, bbox_color=(0, 0, 1)) # 添加gt的3D bbox
        show_path = osp.join(result_path,
                             f'{filename}_online.png') if snapshot else None # 如果截图为True,则拼接截图路径
        vis.show(show_path) # 显示点云和bbox

    if points is not None:
        _write_obj(points, osp.join(result_path, f'{filename}_points.obj')) # 保存点云
    ...
    if gt_bboxes is not None:
        ...
        _write_oriented_bbox(gt_bboxes,
                             osp.join(result_path, f'{filename}_gt.obj')) # 保存gt bbox

    if pred_bboxes is not None:
        # bottom center to gravity center
        pred_bboxes[..., 2] += pred_bboxes[..., 5] / 2 # 将bbox中心点从底面提到中心
        # the positive direction for yaw in meshlab is clockwise
        pred_bboxes[:, 6] *= -1
        _write_oriented_bbox(pred_bboxes,
                             osp.join(result_path, f'{filename}_pred.obj')) # 保存预测3D bbox
...
def show_multi_modality_result(img,
    ...
    result_path = osp.join(out_dir, filename) # 拼接文件输出路径
    mmcv.mkdir_or_exist(result_path) # 创建输出文件夹

    if show:
        show_img = img.copy() # 拷贝图像
        if gt_bboxes is not None:
            show_img = draw_bbox(
                gt_bboxes, show_img, proj_mat, img_metas, color=gt_bbox_color) # 将gt box绘制到图片上
        if pred_bboxes is not None:
            show_img = draw_bbox(
                pred_bboxes,
                show_img,
                proj_mat,
                img_metas,
                color=pred_bbox_color) # 将预测的bbox绘制到图片上
        mmcv.imshow(show_img, win_name='project_bbox3d_img', wait_time=0) # 保存图片

    if img is not None:
        mmcv.imwrite(img, osp.join(result_path, f'{filename}_img.png')) # 保存原始图片

    if gt_bboxes is not None:
        gt_img = draw_bbox(
            gt_bboxes, img, proj_mat, img_metas, color=gt_bbox_color)
        mmcv.imwrite(gt_img, osp.join(result_path, f'{filename}_gt.png')) # 保存gt bbox投影图片

    if pred_bboxes is not None:
        pred_img = draw_bbox(
            pred_bboxes, img, proj_mat, img_metas, color=pred_bbox_color)
        mmcv.imwrite(pred_img, osp.join(result_path, f'{filename}_pred.png')) # 保存预测bbox投影图片
