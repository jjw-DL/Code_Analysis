...
def gaussian_2d(shape, sigma=1):
    ...
    """
    m, n = [(ss - 1.) / 2. for ss in shape] # 2, 2
    y, x = np.ogrid[-m:m + 1, -n:n + 1] # (5, 1)和(1, 5)

    h = np.exp(-(x * x + y * y) / (2 * sigma * sigma)) # (5, 5)
    h[h < np.finfo(h.dtype).eps * h.max()] = 0 # np.finfo(h.dtype).eps:2.220446049250313e-16  h.max():1
    return h
...
def draw_heatmap_gaussian(heatmap, center, radius, k=1):
    ...
    if min(masked_gaussian.shape) > 0 and min(masked_heatmap.shape) > 0:
        # 此时masked_heatmap, masked_gaussian的shape相同
