...
class BaseInstance3DBoxes(object):
    ...
    def overlaps(cls, boxes1, boxes2, mode='iou'):
        ...
        rows = len(boxes1) # 获取bbox1的个数 N
        cols = len(boxes2) # 获取bbox2的个数 M
        if rows * cols == 0:
            return boxes1.tensor.new(rows, cols)

        # height overlap
        overlaps_h = cls.height_overlaps(boxes1, boxes2) # 计算高度方向的overlap

        # obtain BEV boxes in XYXYR format
        boxes1_bev = xywhr2xyxyr(boxes1.bev) # 将bbox由中心+宽高+yaw的形式转换为左上+右下+yaw的形式
        boxes2_bev = xywhr2xyxyr(boxes2.bev)

        # bev overlap 计算bev平面的overlap
        ...
        # 3d overlaps <-- bev的overlap * 高度的overlap
        overlaps_3d = overlaps_bev.to(boxes1.device) * overlaps_h

        volume1 = boxes1.volume.view(-1, 1) # bbox1的box的体积
        volume2 = boxes2.volume.view(1, -1)

        if mode == 'iou':
            # the clamp func is used to avoid division of 0
            iou3d = overlaps_3d / torch.clamp(
                volume1 + volume2 - overlaps_3d, min=1e-8) # 计算3D iou
        else:
