...
class TransFusionBBoxCoder(BaseBBoxCoder):
    def __init__(self,
        ...
        self.pc_range = pc_range # [-54, -54]
        self.out_size_factor = out_size_factor # 8
        self.voxel_size = voxel_size # [0.075, 0.075]
        self.post_center_range = post_center_range # [-61.2, -61.2, -10.0, 61.2, 61.2, 10.0]
        self.score_threshold = score_threshold # 0.0
        self.code_size = code_size # 10

    def encode(self, dst_boxes):
        targets = torch.zeros([dst_boxes.shape[0], self.code_size]).to(dst_boxes.device) # (78, 10)
        targets[:, 0] = (dst_boxes[:, 0] - self.pc_range[0]) / (self.out_size_factor * self.voxel_size[0]) # 网格中心 (78,)
        targets[:, 1] = (dst_boxes[:, 1] - self.pc_range[1]) / (self.out_size_factor * self.voxel_size[1]) # (78,)
    ...
    def decode(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        """
        # class label
        final_preds = heatmap.max(1, keepdims=False).indices # (200,)
        final_scores = heatmap.max(1, keepdims=False).values # (200,)

        # change size to real world metric 真实世界坐标
        center[:, 0, :] = center[:, 0, :] * self.out_size_factor * self.voxel_size[0] + self.pc_range[0] # (1, 200）
        center[:, 1, :] = center[:, 1, :] * self.out_size_factor * self.voxel_size[1] + self.pc_range[1] # (1, 200）
        ...
        if vel is None:
            ...
            final_box_preds = torch.cat([center, height, dim, rot, vel], dim=1).permute(0, 2, 1) # (1, 200, 9)
        ...
        # use score threshold --> self.score_threshold=0
        # 根据预测分数阈值进行过滤
