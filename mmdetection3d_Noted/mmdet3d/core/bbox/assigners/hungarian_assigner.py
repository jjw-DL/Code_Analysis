...
class BBoxBEVL1Cost(object):
    def __init__(self, weight):
        self.weight = weight # 0.25

    def __call__(self, bboxes, gt_bboxes, train_cfg):
        pc_start = bboxes.new(train_cfg['point_cloud_range'][0:2]) # [-54, -54]
        pc_range = bboxes.new(train_cfg['point_cloud_range'][3:5]) - bboxes.new(train_cfg['point_cloud_range'][0:2]) # [108, 108]
        # normalize the box center to [0, 1]
        normalized_bboxes_xy = (bboxes[:, :2] - pc_start) / pc_range # [200, 2]
        normalized_gt_bboxes_xy = (gt_bboxes[:, :2] - pc_start) / pc_range # [78, 2]
        reg_cost = torch.cdist(normalized_bboxes_xy, normalized_gt_bboxes_xy, p=1) # (200, 78) 两个box中心坐标的L1范数
        return reg_cost * self.weight
...
class IoU3DCost(object):
    ...
    def __call__(self, iou):
        iou_cost = - iou # 这里iou取负值的原因是iou越大表示越匹配，损失则越小 eg:(200. 78)
...
class HungarianAssigner3D(BaseAssigner):
    def __init__(self,
        ...
        self.reg_cost = build_match_cost(reg_cost) # BBoxBEVL1Cost
        self.iou_cost = build_match_cost(iou_cost) # IoU3DCost
        self.iou_calculator = build_iou_calculator(iou_calculator) # BboxOverlaps3D
    ...
    def assign(self, bboxes, gt_bboxes, gt_labels, cls_pred, train_cfg):
        ...
        cls_cost = self.cls_cost(cls_pred[0].T, gt_labels) # (200, 10)和(78,)-->(200, 78)
        reg_cost = self.reg_cost(bboxes, gt_bboxes, train_cfg) # (200, 9)和(78, 9)-->(200, 78)
        iou = self.iou_calculator(bboxes, gt_bboxes) # (200, 78)
        iou_cost = self.iou_cost(iou) # (200, 78)
        ...
        matched_row_inds = torch.from_numpy(matched_row_inds).to(bboxes.device) # (78,)
        matched_col_inds = torch.from_numpy(matched_col_inds).to(bboxes.device) # (78,)

        # 4. assign backgrounds and foregrounds
        # assign all indices to backgrounds first
        assigned_gt_inds[:] = 0 # (200, )
        # assign foregrounds based on matching results
        assigned_gt_inds[matched_row_inds] = matched_col_inds + 1 # gt索引规定从1开始
        assigned_labels[matched_row_inds] = gt_labels[matched_col_inds] # 为匹配的gt分配label

        max_overlaps = torch.zeros_like(iou.max(1).values) # iou全0初始化 (200,)
