...
def box3d_multiclass_nms(mlvl_bboxes,
    ...
    num_classes = mlvl_scores.shape[1] - 1 # 10
    # 初始化
    bboxes = []
    scores = []
    labels = []
    ...
    for i in range(0, num_classes):
        ...
        cls_inds = mlvl_scores[:, i] > score_thr # score_thr：0.05 (3479,)
        if not cls_inds.any():
            continue

        _scores = mlvl_scores[cls_inds, i] # 提取该分类分数大于阈值的分数 eg:(340,)
        _bboxes_for_nms = mlvl_bboxes_for_nms[cls_inds, :] #  (340, 5)

        if cfg.use_rotate_nms:
            nms_func = nms_gpu # Nms function with gpu implementation
        else:
            nms_func = nms_normal_gpu

        selected = nms_func(_bboxes_for_nms, _scores, cfg.nms_thr) # 被保留box的索引 eg:(246,)
        _mlvl_bboxes = mlvl_bboxes[cls_inds, :] # 提取该分类分数大于阈值的box
        bboxes.append(_mlvl_bboxes[selected]) # 提取NMS后的bbox eg:(246, 5)
        scores.append(_scores[selected]) # 提取NMS后的分类分数 eg:(246,)
        cls_label = mlvl_bboxes.new_full((len(selected), ),
                                         i,
                                         dtype=torch.long) # 设置类别标签
        labels.append(cls_label) # 将类别标签加入labels

        # 处理方向分类
        if mlvl_dir_scores is not None:
            _mlvl_dir_scores = mlvl_dir_scores[cls_inds] # (340,)
            dir_scores.append(_mlvl_dir_scores[selected]) # (246,)
        # 处理属性
        if mlvl_attr_scores is not None:
            _mlvl_attr_scores = mlvl_attr_scores[cls_inds] # (340,)
            attr_scores.append(_mlvl_attr_scores[selected]) # (246,)
        # 处理2d bbox
    ...
    # -----------------------------
    if bboxes:
        # 拼接NMS后的bboxes,scores,lables,dir_scores以及attr_scores
        bboxes = torch.cat(bboxes, dim=0) # (277, 9)
        scores = torch.cat(scores, dim=0) # (277,)
        labels = torch.cat(labels, dim=0) # (277,)
        if mlvl_dir_scores is not None:
            dir_scores = torch.cat(dir_scores, dim=0) # (277,)
        if mlvl_attr_scores is not None:
            attr_scores = torch.cat(attr_scores, dim=0) # (277,)
        if mlvl_bboxes2d is not None:
            bboxes2d = torch.cat(bboxes2d, dim=0)
        # 如果NMS后的bbox数量比设置的最大bbox数量多
        if bboxes.shape[0] > max_num:
            _, inds = scores.sort(descending=True) # 对scores进行排序
            inds = inds[:max_num] # 取前max个索引
            # 对bboxes,scores,lables,dir_scores以及attr_scores进一步提取
            bboxes = bboxes[inds, :] # (200, 9)
            labels = labels[inds] # (200,)
            scores = scores[inds] # (200,)
            if mlvl_dir_scores is not None:
                dir_scores = dir_scores[inds] # (200,)
            if mlvl_attr_scores is not None:
                attr_scores = attr_scores[inds] # (200,)
            if mlvl_bboxes2d is not None:
...
def circle_nms(dets, thresh, post_max_size=83):
    ...
    ndets = dets.shape[0] # 8
    suppressed = np.zeros((ndets), dtype=np.int32) # 初始化suppressed数组
    keep = []
    # 逐个bbox处理
    for _i in range(ndets):
        i = order[_i]  # start with highest score box 先取出分数最高的det
        if suppressed[i] == 1:  # 被压制则直接跳过
            continue
        keep.append(i) # 见该bbox的id加入keep数组中
        # 遍历剩余的bbox
        for _j in range(_i + 1, ndets):
            ...
            dist = (x1[i] - x1[j])**2 + (y1[i] - y1[j])**2 # 计算中心距离
