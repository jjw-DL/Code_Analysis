...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, post_max_size=None):
    ...
    """
    order = scores.sort(0, descending=True)[1] # 对分类分数进行排序，并取索引

    if pre_maxsize is not None:
        order = order[:pre_maxsize]
    boxes = boxes[order].contiguous() # 将box按照分数索引重新排序 eg:(340, 5)

    keep = torch.zeros(boxes.size(0), dtype=torch.long) # 初始化keep数组
    num_out = iou3d_cuda.nms_gpu(boxes, keep, thresh, boxes.device.index) # thresh：0.8 --> eg:246 保留box的数目
    keep = order[keep[:num_out].cuda(boxes.device)].contiguous() # 保留box的索引
