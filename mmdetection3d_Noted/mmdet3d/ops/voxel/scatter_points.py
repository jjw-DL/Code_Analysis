...
class DynamicScatter(nn.Module):
    ...
    def __init__(self, voxel_size, point_cloud_range, average_points: bool):
        ...
        **Note**: The CPU and GPU implementation get the same output, but
        ...
        self.average_points = average_points # 是否使用平均值计算 True

    def forward_single(self, points, coors):
        reduce = 'mean' if self.average_points else 'max' # mean
    ...
    def forward(self, points, coors):
        ...
        Args:
            input: NC points
        """
        if coors.size(-1) == 3: # 如果只有一帧点云(含batch的coors是4)，则直接计算体素和体素坐标
