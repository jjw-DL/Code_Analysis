...
class CenterPoint(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        """
        outs = self.pts_bbox_head(pts_feats) # 在这里进入centerpoint_head的forward函数，返回list[dict]: Output results for tasks. 6个task_head(每个task head又包含6个分支)
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        outs = self.pts_bbox_head(x) # tupel(List[dict]) dict中有6个字段‘reg’,'height','dim','rot','vel','heatmap'
    ...
    def aug_test_pts(self, feats, img_metas, rescale=False):
        ...
        for x, img_meta in zip(feats, img_metas):
            outs = self.pts_bbox_head(x) # 将feat送入head的到输出特征
    ...
    def aug_test(self, points, img_metas, imgs=None, rescale=False):
        ...
        if pts_feats and self.with_pts_bbox:
            pts_bbox = self.aug_test_pts(pts_feats, img_metas, rescale) # 逐个特征图进行增强Test
            bbox_list.update(pts_bbox=pts_bbox) # 更新预测的box
        return [bbox_list]
