...
class DynamicTransFusionDetector(MVXTwoStageDetector):
    """Base class of Multi-modality VoxelNet."""

    def __init__(self, **kwargs):
        super(DynamicTransFusionDetector, self).__init__(**kwargs)
    ...
    def init_weights(self, pretrained=None):
        ...
        super(DynamicTransFusionDetector, self).init_weights()

        if self.freeze_img:
            if self.with_img_backbone:
                self.img_backbone.init_weights()
                for name, m in self.img_backbone.named_children():
                    m.eval()
                    for param in m.parameters():
                        param.requires_grad = False
            if self.with_img_neck:
                self.img_neck.init_weights()
                for name, m in self.img_neck.named_children():
                    m.eval()
                    for param in m.parameters():
                            param.requires_grad = False
    ...
    def simple_test_pts(self, x, x_img, img_metas, rescale=False):
        ...
        # outs:List[Dict{Tensor:6(center, height, dim, rot, vel, heatmap, query_heatmap_score, dense_heatmap)}]
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False):
        ...
        img_feats, pts_feats = self.extract_feat(
            points, img=img, img_metas=img_metas) # Tuple(Tensor:5)和List(Tensor:1)

        bbox_list = [dict() for i in range(len(img_metas))] # eg:[{}]
