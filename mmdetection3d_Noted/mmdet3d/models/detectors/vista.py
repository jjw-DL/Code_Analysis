...
class VISTA(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.rv_backbone = builder.build_neck(pts_backbone)
        self.rv_neck = builder.build_neck(pts_neck)
        self.ca_neck = builder.build_neck(pts_ca_neck)
    ...
    def extract_pts_feat(self, points, img_feats, img_metas):
        ...
        (x_bev, x_rv) = x
        x_bev = self.pts_backbone(x_bev)
        x_bev = self.pts_neck(x_bev)

        x_rv = self.rv_backbone(x_rv)
        x_rv = self.rv_neck(x_rv)
        
        return self.ca_neck((x_bev[0], x_rv[0]))
    ...
    def forward_pts_train(self,
        ...
        """
        xs, attn_maps = pts_feats # List[Tensor] (4, 384, 160, 160)和(4, 1600, 480)
        outs = self.pts_bbox_head([xs]) # 在这里进入centerpoint_head的forward函数，返回list[dict]: Output results for tasks. 6个task_head(每个task head又包含6个分支)
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs, attn_maps] # 经gt box，gt label和out freature组成list
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        outs = self.pts_bbox_head([x[0]]) # tupel(List[dict]) dict中有6个字段‘reg’,'height','dim','rot','vel','heatmap'
        bbox_list = self.pts_bbox_head.get_bboxes(
            outs, img_metas, rescale=rescale) # List[List[LidarInstance3DBoxes, score(tensor), lable(tensor)],...] 内部的每个list表示一帧的预测结果
        bbox_results = [
