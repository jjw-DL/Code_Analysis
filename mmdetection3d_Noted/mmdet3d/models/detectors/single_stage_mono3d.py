...
class SingleStageMono3DDetector(SingleStageDetector):
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        # 多尺度特征 [(1, 256, 116, 200),(1, 256, 58, 100),(1, 256, 29, 50), (1, 256, 15, 25), (1, 256, 8, 13)]
        x = self.extract_feat(img) 
        outs = self.bbox_head(x)
        bbox_outputs = self.bbox_head.get_bboxes(
            *outs, img_metas, rescale=rescale) # 获得bbox输出bboxes, scores, labels, attrs
        ...
        bbox_list = [dict() for i in range(len(img_metas))] # 初始化bbox list的dict
        for result_dict, img_bbox in zip(bbox_list, bbox_img):
            result_dict['img_bbox'] = img_bbox # 将result赋予对应字段
        if self.bbox_head.pred_bbox2d:
