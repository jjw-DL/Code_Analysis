...
def apply_3d_transformation(pcd, coord_type, img_meta, reverse=False):
    ...
    pcd_rotate_mat = (
        ...
        if 'pcd_rotation' in img_meta else torch.eye(
            3, dtype=dtype, device=device)) # 获取点云的旋转矩阵 (3, 3)

    pcd_scale_factor = (
        img_meta['pcd_scale_factor'] if 'pcd_scale_factor' in img_meta else 1.) # 获取点云缩放尺度 1.0234936 或 1

    pcd_trans_factor = (
        torch.tensor(img_meta['pcd_trans'], dtype=dtype, device=device)
        if 'pcd_trans' in img_meta else torch.zeros(
            (3), dtype=dtype, device=device)) # 获取点云平移因子 [0.4392, 0.2236, -0.1190] 或 [0, 0, 0]
    ...
    # 进行逆变换
    if reverse:
        scale_func = partial(pcd.scale, scale_factor=1.0 / pcd_scale_factor) # 1/scale_factor
        translate_func = partial(pcd.translate, trans_vector=-pcd_trans_factor) # 负的平移方向
        # pcd_rotate_mat @ pcd_rotate_mat.inverse() is not
        # exactly an identity matrix
        # use angle to create the inverse rot matrix neither.
        rotate_func = partial(pcd.rotate, rotation=pcd_rotate_mat.inverse()) # 旋转矩阵的逆
