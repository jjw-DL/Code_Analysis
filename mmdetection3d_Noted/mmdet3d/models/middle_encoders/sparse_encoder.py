...
class SparseEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # 中间stage:[41, 1600, 1408] -> [21, 800, 704] -> [11, 400, 352] -> [5, 200, 176]
        #                   16       ->       32       ->      64       ->      64
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        for encoder_layer in self.encoder_layers:
            #（2, 16, 41, 1600, 1408）(2, 32, 21, 512, 512)
            # (2, 32, 21, 800, 704)  (2, 64, 21, 256, 256)
            # (2, 64, 11, 400, 352)  (2, 128, 11, 128, 128)
            # (2, 64, 5, 200, 176)  (2, 128, 5, 128, 128)
            x = encoder_layer(x)
            encode_features.append(x)

        # for detection head
        # [2, 64, 5, 200, 176] -> [2, 128, 2, 200, 176] 或 (2, 128, 5, 128, 128) --> (2, 128, 2, 128, 128)
        out = self.conv_out(encode_features[-1])
        spatial_features = out.dense()

        N, C, D, H, W = spatial_features.shape # 2, 128, 2, 200, 176
        spatial_features = spatial_features.view(N, C * D, H, W) # (2, 256, 200, 176) 将Voxel压缩至BEV特征图
