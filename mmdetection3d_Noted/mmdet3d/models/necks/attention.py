...
class Cross_Attention_Decouple(nn.Module):
    def __init__(self,
        ...
        self.bev_block_res = self.adjust_res(bev_block_res, bev_size) # (40, 40)和(160, 160) --> (40, 40)
        self.rv_block_res = self.adjust_res(rv_block_res, rv_size) # (40, 12)和(160, 12) --> (40, 12)
    ...
    def adjust_res(self, block_res: Sequence[int], size: Sequence[int]):
        h, w = block_res[0], block_res[1] # 40, 40
        H, W = size[0], size[1] # 160, 160
        assert H % h == 0, f"H must be divisible by h, but got H-{H}, h-{h}"
        assert W % w == 0, f"H must be divisible by h, but got H-{W}, h-{w}"
        h_step = math.ceil(H / h) # 4
        w_step = math.ceil(W / w) # 4
        h_length = math.ceil(H / h_step) # 40
        w_length = math.ceil(W / w_step) # 40
    ...
    def forward(self, x: Sequence[torch.Tensor]):
        ...
        # generate feature block of bev (for attention avg pool) --> (4, 384, 40, 40) 平均池化
