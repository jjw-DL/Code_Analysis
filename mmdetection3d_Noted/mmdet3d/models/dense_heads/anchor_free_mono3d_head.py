...
class AnchorFreeMono3DHead(BaseMono3DDenseHead):
    ...
    def __init__(
        ...
        self.dcn_on_last_conv = dcn_on_last_conv # False
        assert conv_bias == 'auto' or isinstance(conv_bias, bool)
        self.conv_bias = conv_bias # True
        self.use_direction_classifier = use_direction_classifier # True
        self.diff_rad_by_sin = diff_rad_by_sin # True
        self.dir_offset = dir_offset # 0.7854
        self.loss_cls = build_loss(loss_cls) # FocalLoss
        self.loss_bbox = build_loss(loss_bbox) # SmoothL1Loss
        self.loss_dir = build_loss(loss_dir) # CrossEntropyLoss
        self.bbox_code_size = bbox_code_size # 9
        self.group_reg_dims = list(group_reg_dims) # (2, 1, 3, 1, 2)
        self.cls_branch = cls_branch # (256, )
        """
        reg_branch=(
            (256, ),  # offset
            (256, ),  # depth
        ...
        self.pred_velo = pred_velo # True
        self.pred_bbox2d = pred_bbox2d # False
        self.out_channels = [] # (256, 256, 256, 256, -1)
        ...
        self.dir_branch = dir_branch # (256, )
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.conv_cfg = conv_cfg # None
        self.norm_cfg = norm_cfg # norm_cfg=dict(type='GN', num_groups=32, requires_grad=True),
        self.fp16_enabled = False
        self.background_label = (
            num_classes if background_label is None else background_label)
        # background_label should be either 0 or num_classes
        assert (self.background_label == 0
                or self.background_label == num_classes) # 10
        self.pred_attrs = pred_attrs # True
        self.attr_background_label = -1
        self.num_attrs = num_attrs # 9
        if self.pred_attrs:
            self.attr_background_label = num_attrs # 9
            self.loss_attr = build_loss(loss_attr) # CrossEntropyLoss
            self.attr_branch = attr_branch # (256, )

        self._init_layers() # 初始化head的各层
        if init_cfg is None:
            self.init_cfg = dict(
                type='Normal',
                layer='Conv2d',
    ...
    def _init_layers(self):
        ...
        self._init_cls_convs() # 初始化分类层 256-->256-->256-->256-->256
        self._init_reg_convs() # 初始化回归层 256-->256-->256-->256-->256
        self._init_predictor() # 初始化预测层 offset, depth, size, rot, velo
        ...
        for i in range(self.stacked_convs): # 4
            ...
            self.cls_convs.append(
                ConvModule(
                    ...
                    bias=self.conv_bias)) # 256-->256-->256-->256-->256
    ...
    def _init_predictor(self):
        ...
        self.conv_cls_prev = self._init_branch(
            ...
            conv_strides=(1, ) * len(self.cls_branch)) # 256-->256
        ...
        for i in range(len(self.group_reg_dims)):
            reg_dim = self.group_reg_dims[i] # (2, 1, 3, 1, 2) offset, depth, size, rot, velo
            reg_branch_channels = self.reg_branch[i] # (256, 256, 256, 256, None)
            out_channel = self.out_channels[i] # (256, 256, 256, 256, -1)
            if len(reg_branch_channels) > 0:
                self.conv_reg_prevs.append(
                    self._init_branch(
                        conv_channels=reg_branch_channels,
                        conv_strides=(1, ) * len(reg_branch_channels))) # prev
                self.conv_regs.append(nn.Conv2d(out_channel, reg_dim, 1)) # 正式回归
            else:
                self.conv_reg_prevs.append(None) # 针对速度预测，没有prev
                self.conv_regs.append(
                    nn.Conv2d(self.feat_channels, reg_dim, 1)) # 256-->2
        if self.use_direction_classifier:
            self.conv_dir_cls_prev = self._init_branch(
                conv_channels=self.dir_branch,
                conv_strides=(1, ) * len(self.dir_branch)) # 256-->256
            self.conv_dir_cls = nn.Conv2d(self.dir_branch[-1], 2, 1) # 256-->2
        if self.pred_attrs:
            self.conv_attr_prev = self._init_branch(
                conv_channels=self.attr_branch,
                conv_strides=(1, ) * len(self.attr_branch)) # 256-->256
            self.conv_attr = nn.Conv2d(self.attr_branch[-1], self.num_attrs, 1) # 256-->9
    ...
    def forward_single(self, x):
        """Forward features of a single scale levle. 逐个特征层处理
        ...
        """
        cls_feat = x # eg:(2, 256, 116, 200)
        reg_feat = x
        # ---------------------------------
        # 1.分类预测
        # ---------------------------------
        for cls_layer in self.cls_convs:
            cls_feat = cls_layer(cls_feat) # eg:(2, 256, 116, 200) 共享4层卷积
        ...
        for conv_cls_prev_layer in self.conv_cls_prev:
            clone_cls_feat = conv_cls_prev_layer(clone_cls_feat) # eg:(2, 256, 116, 200) 1层卷积
        cls_score = self.conv_cls(clone_cls_feat) # eg:(2, 10, 116, 200)
        # ---------------------------------
        # 2.几何回归
        # ---------------------------------
        for reg_layer in self.reg_convs:
            reg_feat = reg_layer(reg_feat) # eg:(2, 256, 116, 200) 共享4层卷积
        bbox_pred = []
        for i in range(len(self.group_reg_dims)):
            ...
            if len(self.reg_branch[i]) > 0:
                for conv_reg_prev_layer in self.conv_reg_prevs[i]:
                    clone_reg_feat = conv_reg_prev_layer(clone_reg_feat) # eg:(2, 256, 116, 200) 一层前置卷积
            # (2, 1, 3, 1, 2): offset, depth, size, rot, velo
            bbox_pred.append(self.conv_regs[i](clone_reg_feat)) # eg:(2, 2, 116, 200)...(2, 1, 116, 200)
        bbox_pred = torch.cat(bbox_pred, dim=1) # eg:(2, 9, 116, 200)
        ...
        if self.use_direction_classifier:
            ...
            for conv_dir_cls_prev_layer in self.conv_dir_cls_prev:
                clone_reg_feat = conv_dir_cls_prev_layer(clone_reg_feat) # eg:(2, 256, 116, 200)
            dir_cls_pred = self.conv_dir_cls(clone_reg_feat) # eg:(2, 2, 116, 200)
        ...
        if self.pred_attrs:
            ...
            attr_pred = self.conv_attr(clone_cls_feat) # eg:(2, 9, 116, 200)
        """
        cls_score: (2, 10, 116, 200)
        bbox_pred: (2, 9, 116, 200)
        dir_cls_pred: (2, 2, 116, 200)
        attr_pred: (2, 9, 116, 200)
        cls_feat: (2, 256, 116, 200) 经过4层共享卷积后的分类特征
        reg_feat: (2, 256, 116, 200) 经过4层共享卷积后的回归特征
        """
    ...
    def _get_points_single(self,
        ...
        h, w = featmap_size
        x_range = torch.arange(w, dtype=dtype, device=device) # 生成x范围的坐标
        y_range = torch.arange(h, dtype=dtype, device=device) # 生成y范围的坐标
        y, x = torch.meshgrid(y_range, x_range) # meshgrid生成坐标
        if flatten:
            y = y.flatten()
            x = x.flatten()
        return y, x
