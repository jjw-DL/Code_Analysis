...
class VistaSeparateHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(VistaSeparateHead, self).__init__(init_cfg=init_cfg)
...
@HEADS.register_module()
class VistaDCNSeparateHead(BaseModule):
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(VistaDCNSeparateHead, self).__init__(init_cfg=init_cfg)
        if 'heatmap' in heads:
            heads.pop('heatmap')
    ...
    def forward(self, xs):
        ...
        """
        x_sem, x_geo = xs
        center_feat = self.feature_adapt_cls(x_sem) # 特征图经过分类DCN生成分类分支特征图 (4, 64, 160, 160)
        reg_feat = self.feature_adapt_reg(x_geo) # 特征图经过回归DCN生成回归分支特征图 (4, 64, 160, 160)

        cls_score = self.cls_head(center_feat) # 单独处理分类分支 (4, 1or2, 160, 160)
...
class VistaHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 separate_head=dict(
                     type='VistaSeparateHead', init_bias=-2.19, final_kernel=3),
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(VistaHead, self).__init__(init_cfg=init_cfg)
        ...
        self.attn_res = attn_res # [40, 40]
        self.point_cloud_range = point_cloud_range # -51.2, -51.2, -5.0, 51.2, 51.2, 3.0]
        ...
        for task_id, task in enumerate(tasks):
            loss_attn_cfg = dict(type='AttentionConstrainedLoss', num_class=task.num_class,
                                 query_res=self.attn_res, pc_range=self.point_cloud_range)
            self.attn_loss.append(
                build_loss(loss_attn_cfg)
            )
    ...
    def forward_single(self, xs):
        """Forward function for VISTA.
        ...
        for task in self.task_heads:
            ret_dicts.append(task((x_sem, x_geo))) # 返回6个task{(4, 160, 160, 10)和(4, 160, 160, 1)} 
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d):
        ...
        Returns:
            Returns:
                tuple[list[torch.Tensor]]: Tuple of target including \
                    ...
                    list内共6个元素, 每一个元素为一个tensor, 包含该task的batch信息
        ...
        """
        # ([[6-Tensor],[6-Tensor]], [[],[]], [[],[]], [[],[]])
        heatmaps, anno_boxes, inds, masks, task_boxes = multi_apply(
        ...
        heatmaps = [torch.stack(hms_) for hms_ in heatmaps] # hms_(1, 128, 128)在每个list内进行stack --> (2, 1, 128, 128) 或 (2, 2, 128, 128)
        ...
        # Transpose task boxes
        task_boxes = list(map(list, zip(*task_boxes)))
        # task_boxes = [torch.stack(task_boxes_) for task_boxes_ in task_boxes]
        return heatmaps, anno_boxes, inds, masks, task_boxes

    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        """Generate training targets for a single sample. 针对一帧进行处理
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    ind[new_idx] = y * feature_map_size[0] + x # 特征图展开成一维向量的索引赋值给ind，记录该类别的物体在特征图中的位置 eg:ind[0] = 2122
                    mask[new_idx] = 1
        ...
        return heatmaps, anno_boxes, inds, masks, task_boxes # 都是包含6个list的list

    @force_fp32(apply_to=('preds_dicts'))
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, attn_maps, **kwargs):
        ...
        Args:
            ...
            preds_dicts (dict): Output of forward function.
                tuple(list[dict]): Output results for tasks. 6个task_head(每个task head又包含6个分支)
        Returns:
            dict[str:torch.Tensor]: Loss of heatmap and bbox of each task.
        """
        # 生成6个task对应的heatmap, anno_boxes，有效位置索引和有效位置mask
        # heatmap的每个item shape: (2, 1, 160, 160) 或 (2, 2, 160, 160)
        # anno_boxes的每个item shape: (2, 500, 10)
        # inds的每个item shape: (2, 500) 表示有效物体的中心点在特征图展开向量(一维向量)中的索引
        # masks的每个item shape: (2, 500) 表示有效物体mask
        heatmaps, anno_boxes, inds, masks, task_boxes = self.get_targets(
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            # heatmap focal loss
            preds_dict[0]['heatmap'] = clip_sigmoid(preds_dict[0]['heatmap']) # 先计算预测heatmap的sigmoid,这里取0的原因是preds_dict是List[dict]
            ...
            # 2. 计算回归损失 L1 Loss
            target_box = anno_boxes[task_id] # 获取target box eg:(2, 500, 10)
            ...
            loss_bbox = self.loss_bbox(
                pred, target_box, bbox_weights, avg_factor=(num + 1e-4)) # eg：1.9046
            ...
            for index, bam in enumerate(attn_maps):
                norm_var_loss = self.attn_loss[task_id](
                    bam, task_boxes[task_id])
                loss_dict[f'task{task_id}.loss_var_{index}'] = norm_var_loss
    ...
    def get_bboxes(self, preds_dicts, img_metas, img=None, rescale=False):
        ...
        Args:
            preds_dicts (tuple[list[dict]]): Prediction results. 6个task(List)的输出结果，每个dict有6个item
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            assert self.test_cfg['nms_type'] in ['circle', 'rotate'] # rotate
    ...
    def get_task_detections(self, num_class_with_bg, batch_cls_preds,
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            if num_class_with_bg == 1:
                top_scores = cls_preds.squeeze(-1) # （8，）
                top_labels = torch.zeros(
                    cls_preds.shape[0],
                    device=cls_preds.device,
                    dtype=torch.long) # 全0初始化
            ...
            if self.test_cfg['score_threshold'] > 0.0:
                thresh = torch.tensor(
                    ...
                    device=cls_preds.device).type_as(cls_preds) # 0.1
