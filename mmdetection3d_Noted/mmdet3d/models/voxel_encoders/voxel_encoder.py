...
class DynamicSimpleVFE(nn.Module):
    ...
    def __init__(self,
        ...
        self.scatter = DynamicScatter(voxel_size, point_cloud_range, True) # 返回一个类
        self.fp16_enabled = False
    ...
    def forward(self, features, coors):
        ...
        # voxel_feats: [M, C] 
        # coordinates: [M, 4]
        features, features_coors = self.scatter(features, coors) # 计算动态voxle(不会舍弃点)的特征(voxle内取平均值)和坐标
...
class DynamicVFE(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(feat_channels) - 1):
            in_filters = feat_channels[i] # 10-->64
            ...
            vfe_layers.append(
                nn.Sequential(
                    ...
                    nn.ReLU(inplace=True))) # 10-->64和128-->64
        self.vfe_layers = nn.ModuleList(vfe_layers)
        self.num_vfe = len(vfe_layers) # 2
