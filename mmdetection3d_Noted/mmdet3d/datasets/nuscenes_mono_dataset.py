...
class NuScenesMonoDataset(CocoDataset):
    ...
    def __init__(self,
        ...
        self.data_root = data_root # /data/nuscenes
        self.load_interval = load_interval # 1
        self.with_velocity = with_velocity # True
        self.modality = modality 
        self.box_type_3d, self.box_mode_3d = get_box_type(box_type_3d)
        self.eval_version = eval_version # detection_cvpr_2019
        self.use_valid_flag = use_valid_flag # False
        self.bbox_code_size = 9 
        self.version = version # v1.0-trainval
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann['category_id'] not in self.cat_ids:
                continue
            bbox = [x1, y1, x1 + w, y1 + h] # Box 2d
            if ann.get('iscrowd', False):
                gt_bboxes_ignore.append(bbox)
            else:
                gt_bboxes.append(bbox)
                gt_labels.append(self.cat2label[ann['category_id']]) # 类别
                attr_labels.append(ann['attribute_id']) # 属性
                gt_masks_ann.append(ann.get('segmentation', None))
                # 3D annotations in camera coordinates
                bbox_cam3d = np.array(ann['bbox_cam3d']).reshape(1, -1) # cam系下的box 3d 7
                velo_cam3d = np.array(ann['velo_cam3d']).reshape(1, 2) # cam系下的velocity 2
                nan_mask = np.isnan(velo_cam3d[:, 0])
                velo_cam3d[nan_mask] = [0.0, 0.0]
                bbox_cam3d = np.concatenate([bbox_cam3d, velo_cam3d], axis=-1) # 9
                gt_bboxes_cam3d.append(bbox_cam3d.squeeze())
                # 2.5D annotations in camera coordinates
                center2d = ann['center2d'][:2] # img系下的2d中心坐标
                depth = ann['center2d'][2] # img系下的深度，和cam系下相同
        ...
        gt_bboxes_cam3d = CameraInstance3DBoxes(
            ...
            origin=(0.5, 0.5, 0.5)) # 构造CameraInstance3DBoxes
        ...
        ann = dict(
            bboxes=gt_bboxes, # 2D box
            labels=gt_labels, # 类别
            gt_bboxes_3d=gt_bboxes_cam3d, # cam3D box
            gt_labels_3d=gt_labels_3d, # 类别
            attr_labels=attr_labels, # 属性
            centers2d=centers2d, # img系下的2D坐标
            depths=depths, # img系下的深度
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            boxes_per_frame.extend(boxes) # 将同一帧的box加入list
            ...
            cam_boxes3d_for_nms = xywhr2xyxyr(cam_boxes3d.bev) # 将box转换到BEV视角 (360, 5)
            boxes3d = cam_boxes3d.tensor # (360, 9)
            # generate attr scores from attr labels
            attrs = labels.new_tensor([attr for attr in attrs_per_frame]) # (360,)
            # 对所有box进行NMS
            ...
            cam_boxes3d = CameraInstance3DBoxes(boxes3d, box_dim=9) # (101, 9)
            det = bbox3d2result(cam_boxes3d, scores, labels, attrs) # 转换结果
            # ---------------------------------------------------
            # 此时是NMS后的结果
            # Convert the output to the box class in the nuScenes（Camera系下）
            # ---------------------------------------------------
            boxes, attrs = output_to_nusc_box(det)
            # ---------------------------------------------------
            # Convert the box from camera to global coordinate
            ...
            for i, box in enumerate(boxes):
                ...
                attr = self.get_attr_name(attrs[i], name) # 读取属性
                nusc_anno = dict(
            ...
            if sample_token in nusc_annos:
                nusc_annos[sample_token].extend(annos) # 同一个sample_token的annos进行extend
            else:
        ...
        print('Results writes to', res_path) # 写入json文件
    ...
    def _evaluate_single(self,
        ...
        # 运行eval的main方法
        # --------------------------
        nusc_eval.main(render_curves=True) # 运行eval

        # record metrics 记录评价
        metrics = mmcv.load(osp.join(output_dir, 'metrics_summary.json'))
        detail = dict()
        metric_prefix = f'{result_name}_NuScenes' # img_bbox_NuScenes
        ...
        detail['{}/NDS'.format(metric_prefix)] = metrics['nd_score'] # img_bbox_NuScenes/NDS
        detail['{}/mAP'.format(metric_prefix)] = metrics['mean_ap'] # img_bbox_NuScenes/mAP
        return detail
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        if not ('pts_bbox' in results[0] or 'img_bbox' in results[0]):
            ...
            result_files = dict()
            for name in results[0]: # name:img_bbox
                # not evaluate 2D predictions on nuScenes
                if '2d' in name:
                    continue
                print(f'\nFormating bboxes of {name}')
                results_ = [out[name] for out in results] # 将'img_bbox'去除，内部dict重组
                tmp_file_ = osp.join(jsonfile_prefix, name) # 拼接临时文件路径+img_bbox
                result_files.update(
                    {name: self._format_bbox(results_, tmp_file_)}) # {img_bbox:'/tmp/xxx/results_nusc.json'}
    ...
    def evaluate(self,
        ...
        if isinstance(result_files, dict):
            results_dict = dict()
            for name in result_names: # ['img_bbox']
...
def output_to_nusc_box(detection):
    ...
    """
    box3d = detection['boxes_3d'] # eg:(138, 9)
    scores = detection['scores_3d'].numpy() # (138,)
    labels = detection['labels_3d'].numpy() # (138,)
    attrs = None
    if 'attrs_3d' in detection:
        attrs = detection['attrs_3d'].numpy() # (138,)
    # 转换为numpy格式
    box_gravity_center = box3d.gravity_center.numpy() # (138, 3) 计算bbox的引力中心
    box_dims = box3d.dims.numpy() # (138, 3)
    box_yaw = box3d.yaw.numpy() # (138,)
    ...
    for i in range(len(box3d)):
        q1 = pyquaternion.Quaternion(axis=[0, 0, 1], radians=box_yaw[i]) # 绕z轴旋转yaw角度
        q2 = pyquaternion.Quaternion(axis=[1, 0, 0], radians=np.pi / 2) # 绕x轴旋转pi/2
        quat = q2 * q1
...
def cam_nusc_box_to_global(info,
    ...
    for (box, attr) in zip(boxes, attrs):
        # Move box to ego vehicle coord system 将box从cam坐标系转换到自车坐标系
...
def global_nusc_box_to_cam(info,
    ...
    # 逐个box处理
    for box in boxes:
        # Move box to ego vehicle coord system 将box从global坐标系转换到ego坐标系
        box.translate(-np.array(info['ego2global_translation']))
        box.rotate(
        ...
        if radius > det_range:
            continue
        # Move box to camera coord system 将所有box从ego坐标系转换到front相机坐标系
        box.translate(-np.array(info['cam2ego_translation']))
...
def nusc_box_to_cam_box3d(boxes):
    ...
    """
    locs = torch.Tensor([b.center for b in boxes]).view(-1, 3) # 提取box中心位置 eg:(360, 3)
    dims = torch.Tensor([b.wlh for b in boxes]).view(-1, 3) # 提取box的长宽高 (360, 3)
    rots = torch.Tensor([b.orientation.yaw_pitch_roll[0] # 提取box的旋转角 (360, 1)
                         for b in boxes]).view(-1, 1)
    velocity = torch.Tensor([b.velocity[0::2] for b in boxes]).view(-1, 2) # 提取box的速度 (360, 2)

    # convert nusbox to cambox convention
    dims[:, [0, 1, 2]] = dims[:, [1, 2, 0]]
    rots = -rots

    boxes_3d = torch.cat([locs, dims, rots, velocity], dim=1).cuda() # 组合bbox eg:(360, 9)
    cam_boxes3d = CameraInstance3DBoxes(
        boxes_3d, box_dim=9, origin=(0.5, 0.5, 0.5)) # 构造cameraInstance3Dbox
