...
class LyftDataset(Custom3DDataset):
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        """
        lyft_annos = {}
        mapped_class_names = self.CLASSES # 9类

        print('Start to convert detection format...')
        # 逐帧处理
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            annos = []
            boxes = output_to_lyft_box(det) # LyftBox
            sample_token = self.data_infos[sample_id]['token'] # token
            boxes = lidar_lyft_box_to_global(self.data_infos[sample_id], boxes) # 将box从lidar-->ego-->global坐标系
            for i, box in enumerate(boxes):
                ...
                lyft_anno = dict(
                    ...
                    name=name,
                    score=box.score) # 构造lyft_anno
                annos.append(lyft_anno) # 将lyft_anno加入annos列表
            lyft_annos[sample_token] = annos  # 将该帧的标注按照token加入lyft_annos字典
        lyft_submissions = {
            'meta': self.modality, # 构造meta信息
            'results': lyft_annos, # 将lyft_annos赋值到results字段中
        }

        mmcv.mkdir_or_exist(jsonfile_prefix)
        res_path = osp.join(jsonfile_prefix, 'results_lyft.json')
        print('Results writes to', res_path)
        mmcv.dump(lyft_submissions, res_path) # 写入json文件
        return res_path
    ...
    def _evaluate_single(self,
        ...
        # record metrics
        detail = dict()
        metric_prefix = f'{result_name}_Lyft' # pts_bbox_Lyft

        for i, name in enumerate(metrics['class_names']):
            AP = float(metrics['mAPs_cate'][i]) # 获取该类比的AP
            detail[f'{metric_prefix}/{name}_AP'] = AP # 将AP赋值
...
def output_to_lyft_box(detection):
    ...
    """
    box3d = detection['boxes_3d'] # LidarInstance3DBoxes eg:(17, 7)
    scores = detection['scores_3d'].numpy() # eg:(300,)
    labels = detection['labels_3d'].numpy() # eg:(17,)

    box_gravity_center = box3d.gravity_center.numpy() # 获取box的重力中心 (17, 3)
    box_dims = box3d.dims.numpy() # 获取box的长宽和高 (17, 3)
    box_yaw = box3d.yaw.numpy() # 获取yaw角 (17,)
    ...
    for i in range(len(box3d)):
        ...
        box = LyftBox(
            ...
            quat, # 旋转
            label=labels[i], # 类别
            score=scores[i]) # 分数 --> 没有速度赋值，则LyftBox默认为3个nan
        box_list.append(box)
    return box_list
