...
class LoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        img = np.stack(
            [mmcv.imread(name, self.color_type) for name in filename], axis=-1) # (900, 1600, 3, 6)
        if self.to_float32:
            img = img.astype(np.float32)
        results['filename'] = filename # List[6(str)]
        # unravel to list, see `DefaultFormatBundle` in formating.py
        # which will transpose each image separately and then stack into array
        results['img'] = [img[..., i] for i in range(img.shape[-1])] # List[arrray(900, 1600, 3)]
        results['img_shape'] = img.shape # (900, 1600, 3, 6)
        results['ori_shape'] = img.shape # (900, 1600, 3, 6)
        # Set initial values for default meta_keys
        results['pad_shape'] = img.shape # (900, 1600, 3, 6)
        results['scale_factor'] = 1.0
        num_channels = 1 if len(img.shape) < 3 else img.shape[2]
        results['img_norm_cfg'] = dict(
            mean=np.zeros(num_channels, dtype=np.float32), # (0, 0, 0)
            std=np.ones(num_channels, dtype=np.float32), # (1, 1, 1)
            to_rgb=False)
        return results
...
class LoadImageFromFileMono3D(LoadImageFromFile):
    ...
    def __call__(self, results):
        ...
        results['cam2img'] = results['img_info']['cam_intrinsic'] # 相机内参
        return results
...
class LoadAnnotations3D(LoadAnnotations):
    ...
    def _load_bboxes_depth(self, results):
        ...
        """
        results['centers2d'] = results['ann_info']['centers2d'] # load img系的2D像素坐标
        results['depths'] = results['ann_info']['depths'] # load img系下的深度
        return results
    ...
    def _load_attr_labels(self, results):
        ...
        Returns:
            dict: The dict containing loaded label annotations.
        """
        results['attr_labels'] = results['ann_info']['attr_labels'] # 加载属性
        return results
