...
def group_by_key(detections, key):
    ...
    for detection in detections:
        groups[detection[key]].append(detection) # 按照类别整理
    return groups
...
def get_envelope(precisions):
    ...
    for i in range(precisions.size - 1, 0, -1):
        precisions[i - 1] = np.maximum(precisions[i - 1], precisions[i]) # 倒排，取当前值和前一个值的最大值，计算包络点
    return precisions
...
def get_ap(recalls, precisions):
    ...
    recalls = np.concatenate(([0.0], recalls, [1.0])) # 在首尾添加默认值 eg:(872,)
    precisions = np.concatenate(([0.0], precisions, [0.0]))

    precisions = get_envelope(precisions)

    # to calculate area under PR curve, look for points where X axis (recall) changes value
    i = np.where(recalls[1:] != recalls[:-1])[0] # 计算recall改变的点

    # and sum (\Delta recall) * prec
    ap = np.sum((recalls[i + 1] - recalls[i]) * precisions[i + 1]) # 计算pr曲线下的面积-->ap值
    return ap
