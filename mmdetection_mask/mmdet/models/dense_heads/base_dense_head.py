...
class BaseDenseHead(BaseModule, metaclass=ABCMeta):
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            # 基于batch索引从多尺度batch tensor中提取多尺度single image的tensor(选取单张图片的多个尺度特征组成list)
            cls_score_list = select_single_mlvl(cls_scores, img_id) # 选取单张图片的多个尺度特征组成list
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, score_factor, priors) in \
            ...
            bboxes = self.bbox_coder.decode(
                priors, bbox_pred, max_shape=img_shape) # 对预测的box进行解码
    ...
    def forward_train(self,
        ...
        """
        outs = self(x) # 返回的是一个list包含5个元素，包含2个特征图输出
