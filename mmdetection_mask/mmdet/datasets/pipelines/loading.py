...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        img = mmcv.imfrombytes(img_bytes, flag=self.color_type) # 读取图片
        if self.to_float32:
            img = img.astype(np.float32)

        results['filename'] = filename # 图片文件名
        results['ori_filename'] = results['img_info']['filename']
        results['img'] = img # 图片
        results['img_shape'] = img.shape # 图片形状
        results['ori_shape'] = img.shape
        results['img_fields'] = ['img'] # 图片域
        return results
...
class LoadAnnotations:
    ...
    def _load_masks(self, results):
        ...
        h, w = results['img_info']['height'], results['img_info']['width'] # 获取图片的高和宽
        gt_masks = results['ann_info']['masks'] # 获取该张图片的所有mask
        if self.poly2mask:
            gt_masks = BitmapMasks(
                [self._poly2mask(mask, h, w) for mask in gt_masks], h, w) # 逐个mask生成图片
        else:
