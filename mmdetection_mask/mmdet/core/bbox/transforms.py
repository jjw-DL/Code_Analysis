...
def bbox2roi(bbox_list):
    ...
    for img_id, bboxes in enumerate(bbox_list):
        if bboxes.size(0) > 0:
            img_inds = bboxes.new_full((bboxes.size(0), 1), img_id) # 构造id
            rois = torch.cat([img_inds, bboxes[:, :4]], dim=-1) # 将img ind与box拼接
        else:
            rois = bboxes.new_zeros((0, 5))
        rois_list.append(rois)
    rois = torch.cat(rois_list, 0) # 将所有图片的roi拼接
    return rois
...
def bbox2result(bboxes, labels, num_classes):
    ...
    if bboxes.shape[0] == 0:
        ...
        if isinstance(bboxes, torch.Tensor):
            bboxes = bboxes.detach().cpu().numpy() # 转换为numpy格式
            labels = labels.detach().cpu().numpy()
        return [bboxes[labels == i, :] for i in range(num_classes)] # 按照类别分类提取bbox
