...
class BitmapMasks(BaseInstanceMasks):
    ...
    def crop_and_resize(self,
        ...
        num_bbox = bboxes.shape[0] # 获取预测bbox的数量
        fake_inds = torch.arange(
            num_bbox, device=device).to(dtype=bboxes.dtype)[:, None] # 构造伪索引，类似于batch
        rois = torch.cat([fake_inds, bboxes], dim=1)  # Nx5
        rois = rois.to(device=device)
        if num_bbox > 0:
            gt_masks_th = torch.from_numpy(self.masks).to(device).index_select(
                0, inds).to(dtype=rois.dtype) # 计算预测bbox对应的gt mask --> (N, H, W)
            # 调用roi_align到gt_masks_th --> (N, 1, H, W)
            targets = roi_align(gt_masks_th[:, None, :, :], rois, out_shape,
                                1.0, 0, 'avg', True).squeeze(1) # --> (N, 28, 28)
            if binarize:
                resized_masks = (targets >= 0.5).cpu().numpy() # 变为二值mask
            else:
