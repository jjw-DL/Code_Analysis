...
def mask_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list,
    ...
    """
    cfg_list = [cfg for _ in range(len(pos_proposals_list))] # 产生和图片数量相同的cfg_list
    # 逐张图片产生targets
    mask_targets = map(mask_target_single, pos_proposals_list,
                       pos_assigned_gt_inds_list, gt_masks_list, cfg_list)
    mask_targets = list(mask_targets)
    if len(mask_targets) > 0:
        mask_targets = torch.cat(mask_targets) # 将mask target连接 --> (N, 28, 28)
    return mask_targets
...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    Args:
        pos_proposals (Tensor): Positive proposals. (n, 4)
        pos_assigned_gt_inds (Tensor): Assigned GT inds of positive proposals. (n,)
        gt_masks (:obj:`BaseInstanceMasks`): GT masks in the format of Bitmap
            or Polygon.
        cfg (dict): Config dict that indicate the mask size. 28
    ...
    Example:
        ...
        >>> assert mask_targets.shape == (5,) + cfg['mask_size']
    """
    device = pos_proposals.device # 获取设备信息
    mask_size = _pair(cfg.mask_size) # (28, 28)
    binarize = not cfg.get('soft_mask_target', False) # True
    num_pos = pos_proposals.size(0) # 计算正例的数量
    if num_pos > 0:
        proposals_np = pos_proposals.cpu().numpy() # 将pos_proposals转换为numpy格式
        maxh, maxw = gt_masks.height, gt_masks.width # 获取gt mask的高和宽
        proposals_np[:, [0, 2]] = np.clip(proposals_np[:, [0, 2]], 0, maxw) # 对proposals的边框进行截断
        proposals_np[:, [1, 3]] = np.clip(proposals_np[:, [1, 3]], 0, maxh)
        pos_assigned_gt_inds = pos_assigned_gt_inds.cpu().numpy() # 将pos_assigned_gt_inds转换为numpy格式
        """
        该函数主要用于mask_targets计算, 它首先通过assigned_inds将mask与bbox对齐，
        然后通过分配的bbox裁剪mask并调整大小为（mask_h，mask_w）
        """
        ...
        mask_targets = torch.from_numpy(mask_targets).float().to(device) # 将mask_targets转换为tensor
    else:
        mask_targets = pos_proposals.new_zeros((0, ) + mask_size)

    return mask_targets # (N, 28, 28)
